...
### 主要章節
1. [當前架構分析](#1-當前架構分析)
   - 1.1 [正確的指令流程](#11-正確的指令流程應該是)
   - 1.2 [當前問題流程](#12-當前問題流程錯誤)

2. [核心問題確認](#2-核心問題確認)
   - 2.1 [ScannerPlugin 不使用 AICommandCenter](#21-問題-1-scannerplugin-不使用-aicommandcenter)
   - 2.2 [ExploiterPlugin 直接實例化檢測器](#22-問題-2-exploiterplugin-直接實例化檢測器)
   - 2.3 [其他 Plugin 也可能有類似問題](#23-問題-3-其他-plugin-也可能有類似問題)

3. [需要修改的組件](#3-需要修改的組件)
   - 3.1 [核心修改清單](#31-核心修改清單)
   - 3.2 [修改詳細說明](#32-修改詳細說明)

4. [修改優先級與範圍](#4-修改優先級與範圍)
   - 4.1 [必須修改 (Priority 0)](#41-必須修改priority-0--發送端核心)
   - 4.2 [重要但可延後 (Priority 1)](#42-重要但可延後priority-1)
   - 4.3 [接收端暫不處理 (Priority 2)](#43-接收端暫不處理priority-2)

5. [接收端暫不處理的原因](#5-接收端暫不處理的原因)
   - 5.1 [用戶需求分析](#51-您的原話分析)
   - 5.2 [當前接收端狀態](#52-當前接收端狀態)

6. [整合模組的角色](#6-整合模組的角色)
   - 6.1 [需求分析](#61-您的原話分析)
   - 6.2 [整合模組當前狀態](#62-整合模組的當前狀態)
   - 6.3 [整合模組修改策略](#63-整合模組的修改策略)

7. [修改步驟建議](#7-修改步驟建議)
   - 7.1 [第一步: 修改 AICommanderV2](#71-第一步-修改-aicommanderv2基礎設施)
   - 7.2 [第二步: 修改 ScannerPlugin](#72-第二步-修改-scannerplugin掃描功能)
   - 7.3 [第三步: 修改 ExploiterPlugin](#73-第三步-修改-exploiterplugin漏洞利用)
   - 7.4 [第四步: 檢查其他 Plugin](#74-第四步-檢查其他-plugin完整性)
   - 7.5 [第五步: 整合測試](#75-第五步-整合測試端到端)

8. [總結](#8-總結)
   - 8.1 [確認清單](#81-確認清單)
   - 8.2 [修改影響範圍](#82-修改影響範圍)
   - 8.3 [修改後的架構](#83-修改後的架構)
   - 8.4 [下一步計劃](#84-下一步計劃)
