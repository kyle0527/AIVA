    ...
    async def save_experience_sample(self, sample: ExperienceSampleType) -> bool:
        ...
        with self.SessionLocal() as session:
            model = ExperienceSampleModel(
                ...
                vulnerability_type=(
                    sample.learning_tags[0] if sample.learning_tags else "unknown"
                ),
            ...
            for model in query.all():
                ...
                sample = ExperienceSample(
                    ...
                    reward_breakdown=dict(
                        model.actual_result.get("reward_breakdown", {})
                    ),
                    ...
                    duration_ms=(
                        model.metadata.get("duration_ms") if model.metadata else None
                    ),
                    ...
                    learning_tags=(
                        list(model.metadata.get("learning_tags", []))
                        if model.metadata
                        else []
                    ),
                    difficulty_level=(
                        int(model.metadata.get("difficulty_level", 1))
                        if model.metadata
                        else 1
                    ),
                )
