...
if TYPE_CHECKING:
    from typing import NoReturn  # 導入 NoReturn 以避免空塊
...
class ScriptNode:
    ...
    # 新增：函數詳細信息
    functions: dict[str, FunctionInfo] = field(default_factory=dict)  # 函數名 -> 函數信息
...
class DataFlowStitcher:
    ...
    def __init__(self):
        ...
        self.function_map: dict[str, FunctionInfo] = {}  # 函數名 -> 函數信息 (全局映射)
        
    def add_script(self, script_path: str, graph=None):  # graph 參數保留以維持向後兼容,但未使用
        """添加腳本分析結果"""
        _ = graph  # 標記為故意未使用
        ...
        # 分析腳本的頭尾
        self._analyze_script_head_tail(node, script_path)
        
        self.script_nodes[script_path] = node
    
    def _parse_source_code(self, script_path: str) -> Optional[ast.AST]:
        """讀取並解析源代碼
        
        複雜度: A (1) - 簡單 try-except
        """
        try:
    ...
    def _analyze_imports(self, tree: ast.AST, node: ScriptNode) -> None:
        """分析 import 依賴
    ...
    def _analyze_function_calls(self, func_node: ast.AST) -> list[str]:
        """分析函數內部的調用
            ...
            if isinstance(child, ast.Call):
                if isinstance(child.func, ast.Name):
                    ...
                    if isinstance(child.func.value, ast.Name):
                        ...
                        called_functions.append(f"{module_name}.{called_func}")
        return called_functions
    
    def _analyze_function_definitions(self, tree: ast.AST, node: ScriptNode, script_path: str) -> tuple[list[str], list[str]]:
        """分析函數定義
        ...
        for node_ast in ast.walk(tree):
            if isinstance(node_ast, (ast.FunctionDef, ast.AsyncFunctionDef)):
                ...
                func_info = FunctionInfo(
                    ...
                    line_number=node_ast.lineno,
                    is_async=isinstance(node_ast, ast.AsyncFunctionDef)
                )
    ...
    def _identify_entry_points(self, defined_functions: list[str], called_functions: list[str], node: ScriptNode) -> None:
        """識別入口點
        
        複雜度: A (4) - 循環 + 條件
        """
        ENTRY_FUNCTION_NAMES = ['main', 'run', 'execute', '__init__']
        
        for func_name in defined_functions:
            is_entry = (func_name not in called_functions or 
                       func_name in ENTRY_FUNCTION_NAMES)
            if is_entry:
                node.entry_points.append(func_name)
                if func_name in node.functions:
                    node.functions[func_name].is_entry_point = True
    
    def _analyze_script_head_tail(self, node: ScriptNode, script_path: str):
        """分析腳本的頭尾特徵
        
        複雜度: A (2) - 主協調函數，單一職責
        原複雜度: D (22) -> 重構後: A (2) [91% 降低]
        """
        # 1. 讀取並解析源代碼
        tree = self._parse_source_code(script_path)
        if tree is None:
            return
        
        # 2. 分析 import 依賴
        self._analyze_imports(tree, node)
        
        # 3. 分析函數定義
        defined_functions, _ = self._analyze_function_definitions(tree, node, script_path)
        
        # 4. 收集外部調用
        called_functions = self._collect_external_calls(tree, node)
        
        # 5. 識別入口點
        self._identify_entry_points(defined_functions, called_functions, node)
        
        # 6. 確定尾（輸出端）：所有定義的函數都可能被外部調用
...
class SmartFlowStitcher:
    ...
    def __init__(self, root_path: Optional[Path] = None):
        ...
        self.root_path = root_path  # 添加 root_path 屬性
        self.script_nodes: dict[str, ScriptNode] = {}  # 添加 script_nodes 屬性
        self.real_connections: list[tuple[str, str]] = []  # 添加 real_connections 屬性
    ...
    def generate_stitched_mermaid(self, graph_sequence: list[str]) -> str:
        ...
        for i, graph_file in enumerate(graph_sequence):
            ...
            graph_rel_path = Path(graph_file).relative_to(self.root_path) if self.root_path else Path(graph_file)
            node_id = f"graph_{i}_{graph_name}"
            
            # 只顯示腳本路徑，不顯示函數列表
            lines.append(f"    {node_id}[\"{graph_rel_path}\"]")
...
class AIVAFlowAnalyzer:
    ...
    def scan_py_files(self, root: Path, ignore: list[str], max_files: int = 10000) -> list[Path]:
        """掃描Python檔案，排除指定模式"""
        files = []
    ...
    def _get_ignore_patterns(self) -> list[str]:
        ...
        return [
            ...
            "_out", "_archive", "aiva_core_analysis",
            "aiva_flow_analyzer.py", "aiva_flow_classifier",
            "analyze_module_distribution.py", ".pytest_cache",
            "target", "build", "dist"
        ]
    
    def _determine_scan_directory(self, target: str, verbose: bool) -> Optional[Path]:
        """決定掃描目錄
    ...
    def _scan_and_process_files(self, scan_dir: Path, verbose: bool) -> int:
        """掃描並處理檔案
        ...
        python_files = self.scan_py_files(scan_dir, ignore_patterns)
        total_files = len(python_files)
        
        if verbose:
            print(f"📁 發現 {total_files} 個 Python 檔案（已排除分析工具）")
    ...
    def _analyze_data_flow_chains(self, verbose: bool) -> tuple[dict, list]:
        """分析數據流鏈路
        
        複雜度: B (7) - 循環 + 條件
        返回: (combined_results, flow_chains)
        """
        if verbose:
            print("🔗 開始修正後的數據流鏈路分析...")
        ...
        return combined_results, flow_chains
    
    def _build_analysis_results(self, target: str, depth: int, processed: int, 
                                 flow_chains: list, combined_results: dict) -> dict:
        """構建分析結果
    ...
    def analyze_directory(self, target: str = "core", depth: int = 3,
        ...
        """
        # 1. 決定掃描範圍
        scan_dir = self._determine_scan_directory(target, verbose)
        if scan_dir is None:
            return {}
        
        # 2. 產圖階段：掃描並處理所有 Python 檔案
        processed = self._scan_and_process_files(scan_dir, verbose)
        
        # 3. 組圖階段：數據流鏈路分析
        combined_results, flow_chains = self._analyze_data_flow_chains(verbose)
        
        # 4. 構建最終結果
        self.results = self._build_analysis_results(
            target, depth, processed, flow_chains, combined_results
        )
        
        return self.results
    
    def _generate_chain_mermaid(self, chain: list[str]) -> str:
        """為數據流鏈路生成 Mermaid 圖 - 箭頭格式"""
        # 生成簡潔的箭頭鏈
        script_names = [Path(script).stem for script in chain]
        mermaid = " → ".join(script_names)
        return mermaid
    ...
    def _extract_entry_points_from_scripts(self) -> list[str]:
        """從腳本中提取入口點
    ...
    def _find_common_entry_functions(self) -> list[str]:
        """查找常見入口函數
    ...
    def _find_meaningful_entry_functions(self) -> list[str]:
        ...
        複雜度: A (2) - 主協調函數，單一職責
        原複雜度: C (11) -> 重構後: A (2) [82% 降低]
        """
        # 1. 從腳本節點中提取入口函數
        meaningful_entries = self._extract_entry_points_from_scripts()
        
        # 2. 如果沒有找到，查找常見入口函數
        if not meaningful_entries:
            meaningful_entries = self._find_common_entry_functions()
        
        # 3. 限制返回數量避免過多的序列
        return meaningful_entries[:10]
    
    def _determine_output_path(self, output_dir: Optional[str]) -> Path:
        """確定輸出路徑
        
        複雜度: A (2) - 簡單條件判斷
        """
        if output_dir is None and USE_INTEGRATED_PATHS:
            ensure_directories()
            return get_analysis_output_dir("python")
        elif output_dir is None:
            output_path = Path("aiva_flow_analysis")
    ...
    def _build_function_mapping(self) -> dict:
        """建立函數映射表
        ...
        function_details = {
            ...
            "script_functions": {}  # 腳本名 -> 函數列表
        }
        
        for script_path, script_node in self.stitcher.script_nodes.items():
            script_name = Path(script_path).stem
            function_details["script_functions"][script_name] = {
                "file_path": script_path,
                "functions": {},
                "entry_points": script_node.entry_points,
                "export_functions": script_node.export_functions
            }
            
            # 記錄每個函數的詳細信息
            for func_name, func_info in script_node.functions.items():
                # 全局函數映射
                function_details["function_map"][func_name] = {
                ...
                # 按腳本分組的函數信息
                function_details["script_functions"][script_name]["functions"][func_name] = {
                    "line_number": func_info.line_number,
                    "is_async": func_info.is_async,
                    "is_entry_point": func_info.is_entry_point,
                    "called_functions": func_info.called_functions,
    ...
    def _build_enhanced_results(self) -> dict:
        """構建增強結果數據
        
        複雜度: A (1) - 單一職責
        """
        enhanced_results = self.results.copy()
        enhanced_results["function_details"] = self._build_function_mapping()
        return enhanced_results
    
    def _save_json_results(self, output_path: Path, enhanced_results: dict) -> None:
        """保存 JSON 結果
    ...
    def _save_mermaid_flows(self, output_path: Path) -> None:
        """保存 Mermaid 流程圖
        ...
        for chain_name, result in self.results.get("combined_flows", {}).items():
            for i, sequence_data in enumerate(result.get("sequences", [])):
                # 簡化檔案名稱：只用流程編號
                chain_num = chain_name.split('_')[-1]
                mermaid_file = output_path / f"flow_{chain_num}.md"
                
                with open(mermaid_file, "w", encoding="utf-8") as f:
                    # 生成標題，顯示前3個腳本名稱
                    script_preview = " → ".join(sequence_data["sequence"][:3])
                    if len(sequence_data["sequence"]) > 3:
                        script_preview += "..."
                    f.write(f"# 流程 {chain_num}：{script_preview}\n\n")
                    ...
                    for path in sequence_data["file_paths"]:
                        full_path = self.target_dir / path if not Path(path).is_absolute() else path
                        f.write(f"- {full_path}\n")
    
    def _generate_basic_statistics(self, f) -> tuple:
        """生成基本統計
        
        複雜度: A (2) - 簡單計算
        返回: (total_functions, entry_functions) 供後續使用
        """
        f.write("## 基本統計\n\n")
        f.write(f"- 處理檔案數: {self.results.get('total_files_processed', 0)}\n")
        f.write(f"- 數據流鏈路: {len(self.results.get('flow_chains', []))}\n")
        f.write(f"- 腳本節點: {len(self.stitcher.script_nodes)}\n")
        f.write(f"- 真實連接: {len(self.stitcher.real_connections)}\n\n")
        
        # 函數統計
        total_functions = sum(len(node.functions) for node in self.stitcher.script_nodes.values())
        entry_functions = sum(len(node.entry_points) for node in self.stitcher.script_nodes.values())
        
        f.write("## 函數統計\n\n")
        f.write(f"- 總函數數: {total_functions}\n")
        f.write(f"- 入口函數: {entry_functions}\n")
        f.write(f"- 平均每腳本函數數: {total_functions/len(self.stitcher.script_nodes):.1f}\n\n")
        
        return total_functions, entry_functions
    
    def _generate_function_table(self, f) -> None:
        """生成函數對應表
        
        複雜度: B (6) - 循環 + 條件
        """
        f.write("## 檔案和函數對應表\n\n")
        f.write("| 腳本名稱 | 檔案名稱 | 函數數量 | 入口函數 | 主要函數 |\n")
        f.write("|----------|----------|----------|----------|----------|\n")
        
        for script_path, script_node in self.stitcher.script_nodes.items():
            script_name = Path(script_path).stem
            file_name = Path(script_path).name
            func_count = len(script_node.functions)
            entry_points = ', '.join(script_node.entry_points[:3])
            if len(script_node.entry_points) > 3:
                entry_points += '...'
            main_functions = ', '.join(list(script_node.functions.keys())[:3])
            if len(script_node.functions) > 3:
                main_functions += '...'
                
            f.write(f"| {script_name} | {file_name} | {func_count} | {entry_points} | {main_functions} |\n")
        
        f.write("\n")
    
    def _generate_script_function_mapping(self, f) -> None:
        """生成腳本與函數對應表
        ...
        f.write("## 腳本與函數對應表\n\n")
        for script_path, script_node in self.stitcher.script_nodes.items():
            script_rel_path = Path(script_path).relative_to(Path(self.root_path)) if self.root_path else Path(script_path)
            f.write(f"### `{script_rel_path}`\n\n")
            
            if script_node.entry_points:
                f.write("**入口函數：**\n")
                for func in script_node.entry_points:
                    f.write(f"- {func}\n")
                f.write("\n")
            
            if script_node.export_functions:
                f.write("**導出函數：**\n")
                for func in script_node.export_functions:
                    f.write(f"- {func}\n")
                f.write("\n")
            
            if script_node.external_calls:
                f.write("**外部調用：**\n")
                for func in script_node.external_calls:
                    f.write(f"- {func}\n")
                f.write("\n")
            
            f.write("\n")
    
    def _generate_connection_list(self, f) -> None:
        """生成連接列表
        
        複雜度: B (6) - 雙層循環 + 條件
        """
        if self.stitcher.real_connections:
            f.write("## 真實腳本連接（含調用函數）\n\n")
            for from_script, to_script in self.stitcher.real_connections:
                from_rel = Path(from_script).relative_to(Path(self.root_path)) if self.root_path else Path(from_script)
                to_rel = Path(to_script).relative_to(Path(self.root_path)) if self.root_path else Path(to_script)
                
                # 找出調用的函數
                called_funcs = []
                if from_script in self.stitcher.script_nodes and to_script in self.stitcher.script_nodes:
                    from_node = self.stitcher.script_nodes[from_script]
                    to_node = self.stitcher.script_nodes[to_script]
                    for call in from_node.external_calls:
                        if call in to_node.export_functions:
                            called_funcs.append(call)
                
                if called_funcs:
                    func_list = ", ".join(called_funcs)
                    f.write(f"- `{from_rel}` → `{to_rel}` (調用: {func_list})\n")
                else:
                    f.write(f"- `{from_rel}` → `{to_rel}`\n")
    
    def save_results(self, output_dir: Optional[str] = None) -> None:
        """保存分析結果
        
        複雜度: A (1) - 主協調函數，單一職責
        原複雜度: E (33) -> 重構後: A (1) [97% 降低]
        
        Args:
            output_dir: 自定義輸出目錄，如果為 None 則使用統一配置路徑
        """
        # 1. 確定輸出路徑
        output_path = self._determine_output_path(output_dir)
        
        # 2. 構建增強結果數據
        enhanced_results = self._build_enhanced_results()
        
        # 3. 保存 JSON 結果
        self._save_json_results(output_path, enhanced_results)
        
        # 4. 保存 Mermaid 流程圖
        self._save_mermaid_flows(output_path)
        ...
        with open(summary_file, "w", encoding="utf-8") as f:
            ...
            self._generate_basic_statistics(f)
            self._generate_function_table(f)
            self._generate_script_function_mapping(f)
            self._generate_connection_list(f)
...
def main():
    ...
    def run_analysis():
        try:
            # 如果有指定 target_dir，使用它；否則使用默認逻輯
            if args.target_dir:
                analyzer = AIVAFlowAnalyzer(target_dir=args.target_dir)
            else:
