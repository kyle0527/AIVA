...
├── module_explorer.py            # 模組探索器 (待遷移)
├── capability_analyzer.py        # 能力分析器 (待遷移)
├── ast_code_analyzer.py          # AST 代碼解析器 (待創建)
├── knowledge_graph.py            # 知識圖譜構建器 (待創建)
├── self_diagnostics.py           # 自我診斷工具 (待創建)
├── __init__.py                   # 模組入口
└── README.md                     # 本文檔
```
