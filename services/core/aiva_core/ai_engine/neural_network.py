...
class AttentionMechanism:
    ...
    def forward(self, hidden_states: np.ndarray, query: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        """
        計算注意力權重和加權輸出
        ...
        # 計算上下文向量
        context_vector = np.sum(hidden_states * attention_weights[:, np.newaxis], axis=0)
...
class NeuralNetworkBuilder:
    ...
    @staticmethod
    def create_classifier(input_size: int, num_classes: int, hidden_sizes: List[int] = None) -> FeedForwardNetwork:
        """
        創建分類器
    ...
    @staticmethod
    def create_regressor(input_size: int, output_size: int, hidden_sizes: List[int] = None) -> FeedForwardNetwork:
        """
        創建回歸器
