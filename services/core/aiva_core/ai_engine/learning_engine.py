...
class AdamOptimizer(Optimizer):
    ...
    def update(self, network, gradients: dict[str, np.ndarray]):
        ...
        for layer_idx, layer in enumerate(network.layers):
            ...
            # 更新權重的動量
            self.m[layer_idx]["weights"] = (
                self.beta1 * self.m[layer_idx]["weights"]
                + (1 - self.beta1) * layer.weights_grad
            )
            self.v[layer_idx]["weights"] = self.beta2 * self.v[layer_idx]["weights"] + (
                1 - self.beta2
            ) * (layer.weights_grad**2)

            # 更新偏置的動量
            self.m[layer_idx]["biases"] = (
                self.beta1 * self.m[layer_idx]["biases"]
                + (1 - self.beta1) * layer.biases_grad
            )
            self.v[layer_idx]["biases"] = self.beta2 * self.v[layer_idx]["biases"] + (
                1 - self.beta2
            ) * (layer.biases_grad**2)
...
class SupervisedLearner:
    ...
    def __init__(self, network, optimizer: Optimizer, loss_function: str = "mse"):
        ...
        self.loss_function = self.loss_functions.get(
...
class ReinforcementLearner:
    ...
    def __init__(
        ...
        # 訓練統計
        self.training_stats = {"episodes": 0, "total_reward": 0, "average_reward": 0}
...
class TransferLearner:
    ...
    def transfer_weights(
        self, layer_indices: list[int] | None = None, freeze_layers: bool = True
    ):
        """遷移權重
        ...
        if layer_indices is None:
            layer_indices = list(
                range(len(self.source_network.layers) - 1)
            )  # 除了最後一層

        for idx in layer_indices:
            if idx < len(self.target_network.layers):
                # 複製權重
                self.target_network.layers[idx].weights = self.source_network.layers[
                    idx
                ].weights.copy()
                self.target_network.layers[idx].biases = self.source_network.layers[
                    idx
                ].biases.copy()
...
class LearningEngineManager:
    ...
    def create_reinforcement_learner(
        self, name: str, network, optimizer_type: str = "adam", **rl_kwargs
    ...
    def create_online_learner(
        self, name: str, network, optimizer_type: str = "adam", **online_kwargs
    ) -> OnlineLearner:
        """創建在線學習器"""
        if optimizer_type == "adam":
            optimizer = AdamOptimizer()
    ...
    def save_training_session(self, name: str, session_data: dict[str, Any]):
        """保存訓練會話"""
        session_data["timestamp"] = datetime.now(UTC).isoformat()
        session_data["session_id"] = str(uuid4())
        self.training_sessions[name] = session_data

    def get_training_history(self, name: str) -> dict[str, Any] | None:
        """獲取訓練歷史"""
