...
class SQLiDetector(BaseCLITool):
    ...
    def execute(self, url: str, param: Optional[str] = None, **kwargs) -> Dict[str, Any]:
        """執行 SQL 注入檢測"""
        result = {
            "tool": self.name,
            "action": "detect_sqli",
...
class ReportGenerator(BaseCLITool):
    ...
    def execute(self, report_type: str, output_path: Optional[str] = None, **kwargs) -> Dict[str, Any]:
        """生成報告"""
...
def get_tool(name: str) -> Optional[BaseCLITool]:
    """根據名稱獲取工具"""
