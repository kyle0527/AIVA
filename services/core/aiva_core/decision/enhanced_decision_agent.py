...
class Decision:
    ...
    def __init__(self, action: str, params: Dict[str, Any] = None, confidence: float = 0.5):
        self.action = action
...
class EnhancedDecisionAgent:
    ...
    def _initialize_decision_rules(self) -> List[Dict[str, Any]]:
        ...
        return [
            ...
            },
            {
                ...
                "condition": lambda ctx: ctx.attempts_without_success >= self.success_threshold,
            ...
            },
            {
                ...
                "condition": lambda ctx: any("ssh" in str(tool).lower() for tool in ctx.available_tools),
    ...
    def _assess_risk_decision(self, context: DecisionContext) -> Optional[Decision]:
        """基於風險評估的決策"""
    ...
    def _make_experience_driven_decision(self, context: DecisionContext) -> Optional[Decision]:
        """基於經驗的決策"""
        ...
        try:
            ...
            if best_experience["success_score"] > 0.8:
                ...
                decision.reasoning = f"基於類似成功經驗 (成功率: {best_experience['success_score']:.1%})"
                return decision
    ...
    def _find_similar_experiences(self, context: DecisionContext) -> List[Dict[str, Any]]:
        """查找相似的成功經驗"""
    ...
    def _calculate_similarity(self, context: DecisionContext, experience: Dict[str, Any]) -> float:
        """計算上下文與經驗的相似度"""
        similarity = 0.0
    ...
    def _apply_decision_rules(self, context: DecisionContext) -> Optional[Decision]:
        """應用決策規則引擎"""
        
        # 按優先級排序規則
        sorted_rules = sorted(self.decision_rules, key=lambda x: x["priority"], reverse=True)
            ...
            try:
                if rule["condition"](context):
                    ...
                    if decision:
                        ...
                        self.logger.info(f"✅ 觸發規則: {rule['name']} -> {rule['action']}")
                        return decision
    ...
    def _execute_rule_action(self, rule: Dict[str, Any], context: DecisionContext) -> Optional[Decision]:
        """執行規則動作"""
    ...
    def _suggest_alternative_strategy(self, context: DecisionContext) -> str:
        ...
        # 根據失敗次數選擇策略
        strategy_index = min(context.attempts_without_success - self.success_threshold, len(strategies) - 1)
    ...
    def get_decision_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "most_common_decision": max(action_counts, key=action_counts.get) if action_counts else "無",
    ...
    def export_decision_analysis(self, output_path: str = None) -> str:
        ...
        analysis = {
            ...
            "decision_rules": [
                {"name": rule["name"], "description": rule["description"], "priority": rule["priority"]}
