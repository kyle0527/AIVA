...
class SkillGraphBuilder:
    ...
    async def _create_skill_nodes(self, capabilities: list[CapabilityInfo]) -> None:
        ...
        for cap in capabilities:
            ...
            scorecard = None
            try:
                scorecard = await self.capability_registry.get_capability_scorecard(
                    cap.id
                )
        ...
        for node_id, node in self.skill_nodes.items():
            for prereq in node.prerequisites:
                ...
                for other_id, other_node in self.skill_nodes.items():
                    if (
                        other_id != node_id
                        and prereq.lower() in other_node.name.lower()
                    ):
                        edge = SkillEdge(
                            source=other_id,
                            target=node_id,
    ...
    async def _analyze_tag_similarity_relationships(self) -> None:
        ...
        for i, node1 in enumerate(nodes_list):
            for node2 in nodes_list[i + 1 :]:
                ...
                if common_tags:
                    similarity = len(common_tags) / max(
                        len(node1.tags), len(node2.tags), 1
                    )
    ...
    async def _analyze_topic_relationships(self) -> None:
        ...
        for source_topic, related_topics in topic_relationships.items():
            source_nodes = [
                n for n in self.skill_nodes.values() if source_topic in n.topic.lower()
            ]

            for related_topic in related_topics:
                target_nodes = [
                    n
                    for n in self.skill_nodes.values()
                    if related_topic in n.topic.lower()
                ]
...
class SkillGraphAnalyzer:
    ...
    def find_optimal_path(
        self, start_capability: str, goal: str, max_path_length: int = 5
    ) -> list[SkillPath]:
        """尋找最佳執行路徑"""
        paths = []
                ...
                try:
                    ...
                    if nx.has_path(self.graph, start_capability, target):
                        shortest_path = nx.shortest_path(
                            self.graph, start_capability, target, weight="weight"
                        )
    ...
    def _create_skill_path(self, node_path: list[str]) -> SkillPath:
        ...
        # 生成路徑描述
        description = " -> ".join(
            [self.skill_nodes[node_id].name for node_id in node_path]
        )
    ...
    def get_capability_recommendations(
        self, capability_id: str, limit: int = 5
        ...
        try:
            ...
            try:
                ...
                pagerank_scores = nx.pagerank(
                    self.graph, personalization={capability_id: 1.0}
                )
...
class AIVASkillGraph:
    ...
    def __init__(self, capability_registry: CapabilityRegistry | None = None):
        ...
        self.analyzer: SkillGraphAnalyzer | None = None
        self.last_build_time: datetime | None = None
    ...
    def get_graph_statistics(self) -> dict[str, Any]:
        ...
        return {
            ...
            "last_build_time": (
                self.last_build_time.isoformat() if self.last_build_time else None
            ),
        }
