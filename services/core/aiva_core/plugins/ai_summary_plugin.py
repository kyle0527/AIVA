...
class AISummaryPlugin:
    ...
    def __init__(self, enabled: bool = True):
        ...
        logger.info(
            f"🔌 {self.plugin_name} v{self.version} {'已啟用' if enabled else '已禁用'}"
        )
    ...
    async def generate_summary(
        ...
        try:
            ...
            summary_prompt = self._build_summary_prompt(
                summary_data, task_analysis, result
            )
            ...
            summary = {
                ...
                "basic_info": {
                    ...
                    "complexity_level": self._get_complexity_level(
                        task_analysis.get("complexity_score", 0)
                    ),
    ...
    def _build_summary_prompt(
        self, summary_data: dict, task_analysis: dict, result: dict
    ) -> str:
        """構建摘要生成提示"""
        return f"""
