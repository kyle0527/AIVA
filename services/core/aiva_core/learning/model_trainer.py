...
class ModelTrainer:
    ...
    async def train_dqn(
        ...
        if self.dqn_trainer is None:
            self.dqn_trainer = DQNTrainer(
                ...
                learning_rate=(
                    config.learning_rate if hasattr(config, "learning_rate") else 0.001
                ),
                gamma=0.99,
                epsilon_start=1.0,
                epsilon_end=0.01,
        ...
        result = ModelTrainingResult(
            ...
            metrics={
                **dqn_metrics,
                ...
                "max_episode_reward": (
                    float(np.max(total_rewards)) if total_rewards else 0.0
                ),
                "min_episode_reward": (
                    float(np.min(total_rewards)) if total_rewards else 0.0
                ),
                "algorithm": "DQN",
            },
            model_path=str(model_path),
    ...
    async def train_ppo(
        ...
        if self.ppo_trainer is None:
            self.ppo_trainer = PPOTrainer(
                ...
                action_dim=action_dim,
                learning_rate=(
                    config.learning_rate if hasattr(config, "learning_rate") else 3e-4
                ),
                gamma=0.99,
                gae_lambda=0.95,
                clip_epsilon=0.2,
                ...
                ppo_epochs=4,
                mini_batch_size=(
                    config.batch_size if hasattr(config, "batch_size") else 64
                ),
            )
        ...
        for episode_idx, episode in enumerate(episodes):
            ...
            if (episode_idx + 1) % 10 == 0:
                avg_reward = np.mean(total_rewards[-10:])
                recent_metrics = (
                    update_metrics_history[-10:] if update_metrics_history else []
                )
                avg_policy_loss = (
    ...
    async def _train_model_supervised(
        ...
        try:
            ...
            model_type = (
                config.model_type if hasattr(config, "model_type") else "random_forest"
            )
    ...
    async def _train_model_rl(
        ...
        try:
            ...
            if mid_point > 0:
                ...
                improvement_rate = (
                    ((late_avg - early_avg) / abs(early_avg)) if early_avg != 0 else 0.0
                )
            else:
            ...
            for episode in episodes:
                ...
                for i, (state, action, reward) in enumerate(
                    zip(states, actions, rewards, strict=False)
                ):
            ...
            self.current_model = {
                ...
                "config": {"learning_rate": 0.1, "discount_factor": 0.95},
            }
    ...
    async def _evaluate_model(
        ...
        try:
            ...
            if isinstance(self.current_model, dict):
                # 強化學習模型（Q-learning）
                logger.warning(
                    "RL model evaluation not fully implemented, returning placeholder"
                )
                return {
                    "model_type": "rl",
    ...
    async def load_model(self, version: str) -> bool:
        ...
        try:
            ...
            # 驗證模型完整性
            if self.current_model is None:
                logger.error("Loaded model data is invalid")
                return False
