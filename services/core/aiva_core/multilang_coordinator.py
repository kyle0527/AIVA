...
class MultiLanguageAICoordinator:
    ...
    async def execute_task(self, task: str, language: Optional[ProgrammingLanguage] = None, **kwargs) -> Dict[str, Any]:
        """
        執行 AI 任務
    ...
    def _initialize_rust_module(self) -> None:
        ...
        try:
            ...
            # 檢查是否有 Rust 執行檔案
            rust_module_path = "services/features/rust_ai_module/target/release/ai_processor"
            try:
                result = subprocess.run([rust_module_path, "--version"], 
                                      capture_output=True, 
                                      timeout=5)
    ...
    def _initialize_typescript_module(self) -> None:
        ...
        try:
            ...
            ts_service_url = "http://localhost:3001/api/health"  # TypeScript AI 服務端點
        ...
        try:
            ...
            if process.returncode == 0:
                ...
                log_cross_language_call(
                    logger, "python", "rust", task, kwargs, result, None, 
                ...
                log_cross_language_call(
                    logger, "python", "rust", task, kwargs, None, error_msg,
                    time.time() - start_time if 'start_time' in locals() else None
                )
    ...
    async def call_go_ai(self, task: str, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                log_cross_language_call(
                    logger, "python", "go", task, kwargs, result, None,
                ...
                log_cross_language_call(
                    logger, "python", "go", task, kwargs, None, error_msg,
                    time.time() - start_time
                )
                return {"success": False, "error": error_msg}
    ...
    async def call_typescript_ai(self, task: str, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            if response.status_code == 200:
                result = response.json()
                log_cross_language_call(
                    logger, "python", "typescript", task, kwargs, result, None,
                    time.time() - start_time
                )
                return {
                    "success": True,
                ...
                error_msg = f"HTTP {response.status_code}: {response.text}"
                log_cross_language_call(
                    logger, "python", "typescript", task, kwargs, None, error_msg,
                    time.time() - start_time
                )
                return {"success": False, "error": error_msg}
