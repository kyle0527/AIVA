...
class IntegrationSettings:
    ...
    def load(cls) -> "IntegrationSettings":
        ...
        return cls(
            ...
            # 認證和安全
            api_token=os.getenv("INTEGRATION_TOKEN"),
            cors_origins=cors_origins,
            # 速率限制
            rate_limit_rps=float(os.getenv("RATE_LIMIT_RPS", "20.0")),
            rate_limit_burst=int(os.getenv("RATE_LIMIT_BURST", "60")),
            # 監控
            enable_prometheus=os.getenv("ENABLE_PROM", "1") != "0",
            # 存儲模式
            storage_mode=os.getenv("STORE", "auto").lower(),
        )
    ...
    def validate(self) -> None:
        ...
        if self.use_sql_storage and not self.db_url:
            raise ValueError("SQL storage mode requires DATABASE_URL")
