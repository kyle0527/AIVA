...
class FileDataSource:
    ...
    async def read_data(self) -> AsyncIterator[DataRecord]:
        ...
        try:
            ...
            if self.format_type == "json":
                ...
                async for record in self._yield_text_records(content):
                    yield record

        except Exception as e:
            self.logger.error(f"讀取文件失敗: {e}")

    async def _yield_json_records(self, content: str) -> AsyncIterator[DataRecord]:
        """從JSON內容產生數據記錄"""
        try:
    ...
    async def _yield_text_records(self, content: str) -> AsyncIterator[DataRecord]:
        """從文本內容產生數據記錄"""
        lines = content.split('\n')
    ...
    async def write_data(self, record: DataRecord) -> bool:
        ...
        try:
            if self._file_handle is None:
                ...
                # 使用 asyncio.to_thread 在線程池中執行 I/O 操作
                self._file_handle = await asyncio.to_thread(
                    open, self.file_path, mode, encoding="utf-8"
                )
            ...
            if self.format_type == "json":
                ...
                await asyncio.to_thread(self._file_handle.write, "\n")
            elif self.format_type == "text":
                await asyncio.to_thread(self._file_handle.write, f"{record.data}\n")
...
class DataPipeline:
    ...
    def __init__(
        ...
        # 並發控制
        max_tasks = self.config_manager.get("pipeline.max_concurrent_tasks", 10)
        batch_size = self.config_manager.get("pipeline.batch_size", 100)
        ...
        if name in self.pipeline_tasks:
            ...
            if not task.done():
                ...
                try:
                    ...
                    self.logger.debug(f"管道任務已取消: {name}")
