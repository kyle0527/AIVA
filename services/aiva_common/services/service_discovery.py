...
class ServiceRegistry:
    ...
    def discover_services(
        ...
        """
        # 獲取候選服務
        candidates = self._get_service_candidates(service_name, tags)

        # 過濾並返回結果
        results = self._filter_services(candidates, status, healthy_only)

        # 按權重排序
        results.sort(key=lambda s: s.weight, reverse=True)
        return results

    def _get_service_candidates(
        self, service_name: str | None, tags: set[str] | None
    ) -> set[str]:
        """獲取候選服務ID集合"""
        # 根據服務名過濾
        if service_name:
        ...
        # 根據標籤過濾
        if tags:
            tag_matches = self._get_tag_matches(tags)
            candidates &= tag_matches

        return candidates

    def _get_tag_matches(self, tags: set[str]) -> set[str]:
        """獲取匹配所有標籤的服務ID集合"""
        tag_matches = set()
        for tag in tags:
            if tag in self._tag_index:
    ...
    def cleanup_stale_services(self, max_age: int = 300) -> int:
        ...
        for service_id in stale_services:
            task = asyncio.create_task(self.deregister_service(service_id))
