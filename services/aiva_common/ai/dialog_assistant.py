...
class DialogSession(BaseModel):
    ...
    def is_expired(self, timeout_minutes: int = 30) -> bool:
        """檢查會話是否過期"""
        return (datetime.now(UTC) - self.last_activity).total_seconds() > (timeout_minutes * 60)
...
class AIVADialogAssistant(IDialogAssistant):
    ...
    async def _get_or_create_session(self, user_id: str) -> DialogSession:
        ...
        if current_session and not current_session.is_expired(self.config.session_timeout_minutes):
    ...
    async def _get_current_session(self, user_id: str) -> Optional[DialogSession]:
        """獲取用戶當前會話"""
        if user_id not in self.user_sessions:
            return None
    ...
    async def _check_consecutive_unclear(self, session: DialogSession) -> None:
        ...
        unclear_count = sum(1 for turn in recent_turns if turn.intent == DialogIntent.UNCLEAR)
