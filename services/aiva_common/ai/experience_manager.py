...
class SQLiteExperienceStorage:
    ...
    async def get_experiences(
        self, filter_params: ExperienceFilter
            ...
            try:
                ...
                for row in rows:
                    ...
                    sample = ExperienceSample(
                        ...
                        reward_breakdown=json.loads(
                            row_dict["reward_breakdown"] or "{}"
                        ),
...
class AIVAExperienceManager(IExperienceManager):
    ...
    def __init__(self, config: ExperienceManagerConfig | None = None):
        """初始化經驗管理器
        ...
        if self.config.storage_backend == "sqlite":
            ...
            raise ValueError(
                f"Unsupported storage backend: {self.config.storage_backend}"
            )
        ...
        logger.info(
            f"AIVAExperienceManager initialized with {self.config.storage_backend} backend"
        )

    async def create_learning_session(self, session_config: dict[str, Any]) -> str:
        """創建學習會話
        ...
        try:
            ...
            if self.config.deduplication_enabled:
                ...
                if sample_hash in self._dedup_cache:
                    logger.debug(
                        f"Duplicate sample detected, skipping: {experience.sample_id}"
                    )
                    return False
            ...
            if experience.quality_score is None:
                experience.quality_score = await self._evaluate_sample_quality(
                    experience
                )
            ...
            if success:
                ...
                event = AIExperienceCreatedEvent(
                    ...
                    vulnerability_type=experience.target_info.get(
    ...
    async def retrieve_experiences(
        ...
        try:
            ...
            quality_threshold = query_context.get(
                "quality_threshold", similarity_threshold
            )
    ...
    async def evaluate_sample_quality(self, sample: ExperienceSample) -> float:
        """評估樣本質量
    ...
    async def cleanup_old_experiences(
        ...
        try:
            ...
            # 獲取要清理的樣本
            filter_params = ExperienceFilter(
                end_time=cutoff_date, sort_by="timestamp", sort_desc=False
            )
    ...
    async def end_learning_session(self, session_id: str) -> bool:
        """結束學習會話
    ...
    def _start_cleanup_task(self) -> None:
        ...
        if self.config.auto_cleanup and (
            ...
            try:
                ...
                # 沒有事件循環在運行，跳過任務創建
                logger.info(
                    "No running event loop, skipping periodic cleanup task creation"
                )
        ...
        while True:
            try:
                ...
                if self.config.auto_cleanup:
                    ...
                    if cleaned_count > 0:
                        logger.info(
                            f"Periodic cleanup: {cleaned_count} samples cleaned"
                        )
...
def create_experience_manager(
    config: ExperienceManagerConfig | None = None, **kwargs  # type: ignore
