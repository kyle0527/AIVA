...
class AIVACapabilityEvaluator(ICapabilityEvaluator):
    ...
    def __init__(
        ...
        self.evidences: Dict[str, List[CapabilityEvidence]] = {}  # capability_id -> evidences
    ...
    async def evaluate_capability(
        ...
        try:
            ...
            logger.info(f"Capability evaluation completed: {capability_id}, score: {assessment.overall_score:.1f}")
            ...
            logger.error(f"Error evaluating capability {capability_id}: {e}", exc_info=True)
            return {"error": str(e)}
    ...
    async def run_benchmark(
        ...
        try:
            ...
            # 獲取測試列表
            if test_suite:
                tests = [self.benchmark_tests[test_id] for test_id in test_suite if test_id in self.benchmark_tests]
            else:
                # 獲取該能力的所有測試
                tests = [test for test in self.benchmark_tests.values() if test.capability_id == capability_id]
            ...
            logger.info(f"Benchmark completed for {capability_id}: {len(results)} tests, avg_score: {avg_score}")
            ...
            logger.error(f"Error running benchmark for capability {capability_id}: {e}", exc_info=True)
            return {"error": str(e)}
        ...
        try:
            ...
            result: Dict[str, Any] = {
                ...
                "evidence_summary": {
                    ...
                    "verified_evidences": sum(1 for e in self.evidences.get(capability_id, []) if e.verified),
                    "recent_evidences": len([e for e in self.evidences.get(capability_id, []) 
                                           if (datetime.now(UTC) - e.timestamp).days <= 7]),
                },
                "benchmark_summary": {
                    "total_results": len(self.benchmark_results.get(capability_id, [])),
                    "recent_results": len([r for r in self.benchmark_results.get(capability_id, []) 
            ...
            if scorecard:
                result["scorecard"] = {
                    ...
                    "last_used_at": scorecard.last_used_at.isoformat() if scorecard.last_used_at else None,
                    "last_updated_at": scorecard.last_updated_at.isoformat() if scorecard.last_updated_at else None,
                }
    ...
    async def add_benchmark_test(
        ...
        try:
            ...
            logger.info(f"Benchmark test added: {test.test_id} for capability {test.capability_id}")
            return True
    ...
    async def _perform_assessment(
        ...
        # 生成建議
        recommendations = await self._generate_recommendations(capability_id, dimension_scores, evidences)
        ...
        assessment = CapabilityAssessment(
            ...
            high_confidence_evidences=len([e for e in evidences if e.confidence >= 0.8]),
    ...
    def _extract_score_from_evidence(self, evidence: CapabilityEvidence) -> float:
        ...
        if isinstance(value, (int, float)):
            ...
            if evidence.evidence_type == "measurement":
                # 性能測量：假設較小值較好 (如延遲)
                if "latency" in evidence.evidence_description.lower() or "time" in evidence.evidence_description.lower():
