...
class AIVACrossLanguageBridge(ICrossLanguageBridge):
    ...
    async def execute_subprocess(
        ...
        try:
            ...
            return self._create_error_result(
                f"Unsupported language: {language}", "UNSUPPORTED_LANGUAGE"
            )
        ...
        try:
            ...
            # 嘗試解析 JSON 輸出
            if (
                result["success"] and str(result["stdout"]).strip()
                if result["stdout"]
                else ""
            ):
                try:
                    result["parsed_output"] = json.loads(
            ...
            logger.error(
                f"Subprocess execution {execution_id} failed: {e}", exc_info=True
            )
    ...
    async def sync_results(
        self, source_language: str, target_language: str, data: dict[str, Any]
    ) -> dict[str, Any]:
        """同步跨語言結果
    ...
    async def create_cross_language_analysis(
        self, project_name: str, codebase: MultiLanguageCodebase
    ) -> CrossLanguageAnalysis:
        """創建跨語言分析
        ...
        for lang, executor in self.config.language_executors.items():
            try:
                ...
                result = subprocess.run(
                    [executor, "--version"], capture_output=True, timeout=5.0, text=True
                )
    ...
    def _build_command(
        self, language: ProgrammingLanguage, executable_path: str, args: list[str]
    ) -> list[str]:
        """構建執行命令"""
    ...
    async def _convert_data_format(
        ...
        for key, value in data.items():
            # 處理不同語言的命名慣例
            converted_key = self._convert_naming_convention(
    ...
    def _convert_data_type(
        ...
        if isinstance(value, dict):
            ...
            for k, v in dict_value.items():  # type: ignore
                if isinstance(k, str):
                    converted_key = self._convert_naming_convention(
                        k, source_lang, target_lang
                    )
                    converted_dict[converted_key] = self._convert_data_type(
                        v, source_lang, target_lang
                    )
                else:
                    # 非字符串鍵直接轉換為字符串
                    key_str = str(k) if k is not None else ""
                    converted_key = self._convert_naming_convention(
                        key_str, source_lang, target_lang
                    )
                    converted_dict[converted_key] = self._convert_data_type(
                        v, source_lang, target_lang
                    )
    ...
    async def _validate_converted_data(
        self, data: dict[str, Any], target_lang: ProgrammingLanguage
    ) -> dict[str, Any]:
        """驗證轉換後的數據"""
        errors = []
        warnings = []
    ...
    def _determine_interop_method(
        self, source_lang: ProgrammingLanguage, target_lang: ProgrammingLanguage
    ) -> str:
        """確定互操作方法"""
        interop_matrix = {
    ...
    def _generate_cross_language_recommendations(
        self, languages: list[ProgrammingLanguage], codebase: MultiLanguageCodebase
    ) -> list[str]:
        """生成跨語言建議"""
        recommendations = []
    ...
    def _create_error_result(
        self, error_message: str, error_code: str, execution_id: str | None = None
    ) -> dict[str, Any]:
        """創建錯誤結果"""
        return {
    ...
    def _create_timeout_result(
        self, execution_id: str, timeout: float
    ) -> dict[str, Any]:
        """創建超時結果"""
        return {
            "execution_id": execution_id,
...
def create_cross_language_bridge(
    config: BridgeConfig | None = None, **kwargs  # type: ignore
