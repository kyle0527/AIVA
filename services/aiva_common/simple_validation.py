...
def test_schema_imports():
    ...
    try:
        ...
        from aiva_common.schemas.threat_intelligence import (
            AttackPattern, Indicator, Bundle, ThreatIntelligenceReport
        )
        
        # 測試API標準
        print("   - 測試API標準模組導入...")
        from aiva_common.schemas.api_standards import (
            OpenAPIDocument, OpenAPIInfo, AsyncAPIDocument, GraphQLSchema
        )
        
        # 測試低價值漏洞
        print("   - 測試低價值漏洞模組導入...")
        from aiva_common.schemas.low_value_vulnerabilities import (
            VulnerabilityPattern, BugBountyStrategy, ROIAnalysis
        )
...
def test_enum_imports():
    ...
    try:
        ...
        from aiva_common.enums.security import (
            STIXObjectType, BugBountyCategory, LowValueVulnerabilityType
        )
        
        # 測試Web標準枚舉
        print("   - 測試Web標準枚舉導入...")
        from aiva_common.enums.web_api_standards import (
            HTTPStatusCode, OpenAPISchemaType
        )
...
def test_bug_bounty_enums():
    ...
    try:
        from aiva_common.enums.security import (
            BugBountyCategory, BountyPriorityTier, 
            VulnerabilityDifficulty, LowValueVulnerabilityType
        )
...
def test_stix_enums():
    ...
    try:
        from aiva_common.enums.security import (
            STIXObjectType, STIXRelationshipType, TLPMarking, 
            CVSSVersion, MITREPlatform
        )
...
def test_hackerone_strategy():
    ...
    try:
        ...
        from aiva_common.enums.security import LowValueVulnerabilityType, ProgramType, ResponseTimeCategory
        ...
        print(f"   - 資源分配: {strategy.low_value_allocation_percent}%/{strategy.high_value_allocation_percent}%")
