...
class PluginMetadata(BaseModel):
    ...
    max_aiva_version: str | None = Field(None, description="最大 AIVA 版本")
...
def plugin_hook(name: str | None = None):
    """插件鉤子裝飾器"""
...
class PluginManager:
            ...
            for plugin_file in plugin_path.glob("**/plugin.toml"):
                try:
                    ...
                    self._logger.warning(
                        f"Failed to load plugin metadata from {plugin_file}: {e}"
                    )
    ...
    async def load_plugin(self, metadata: PluginMetadata) -> BasePlugin | None:
        """加載插件"""
    ...
    async def cleanup_all(self) -> None:
        ...
        for plugin in self._plugins.values():
            try:
                ...
                self._logger.error(
                    f"Failed to cleanup plugin {plugin.metadata.name}: {e}"
                )
