    ...
    def __init__(self, config: CLIConfig | None = None):
        ...
        if RICH_AVAILABLE:
            self.console = Console(
                width=self.config.width, force_terminal=self.config.color
            )
        else:
            self.console = None
...
class CLIProgressBar:
    ...
    def start_task(self, description: str, total: int | None = None):
        """開始任務"""
        if self._progress:
    ...
    def update_task(self, advance: int = 1, description: str | None = None):
        """更新任務進度"""
...
def create_cli_group(
    name: str = "aiva", help_text: str = "AIVA Command Line Interface"
):
    """創建 CLI 組"""
    if not click:
