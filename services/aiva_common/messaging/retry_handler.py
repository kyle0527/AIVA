...
class RetryPolicy(BaseModel):
    ...
    dead_letter_exchange: str = Field(
        default="aiva.dead_letter", description="死信交換機"
    )
...
class MessageMetadata(BaseModel):
    ...
    first_attempt: str = Field(
        default_factory=lambda: datetime.now(UTC).isoformat(),
        description="首次處理時間",
    )
    last_attempt: str = Field(
        default_factory=lambda: datetime.now(UTC).isoformat(),
        description="最後重試時間",
    )
    error_history: list[str] = Field(default_factory=list, description="錯誤歷史")
    original_routing_key: str | None = Field(default=None, description="原始路由鍵")
...
class RetryHandler:
    ...
    def __init__(self, policy: RetryPolicy = None):
        ...
        logger.info(
            f"RetryHandler initialized with policy: max_attempts={self.policy.max_attempts}"
        )
    ...
    def get_retry_headers(
        self, original_headers: dict[str, Any], error: Exception
    ) -> dict[str, Any]:
        """獲取重試消息的頭部信息
        ...
        delay = min(
            ...
            * (self.policy.backoff_factor ** (metadata.retry_count - 1)),
        )
    ...
    def get_dead_letter_headers(
        self, original_headers: dict[str, Any], final_error: Exception
    ) -> dict[str, Any]:
        """獲取死信消息的頭部信息
...
FAST_RETRY_POLICY = RetryPolicy(
    max_attempts=2, backoff_base=0.5, backoff_factor=2.0, max_backoff=5.0
)

STANDARD_RETRY_POLICY = RetryPolicy(
    max_attempts=3, backoff_base=1.0, backoff_factor=2.0, max_backoff=30.0
)

AGGRESSIVE_RETRY_POLICY = RetryPolicy(
    max_attempts=5, backoff_base=2.0, backoff_factor=1.5, max_backoff=60.0
)
