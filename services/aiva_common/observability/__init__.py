...
class AIVALogger:
    ...
    def _create_structured_log(
        self, level: LogLevel, message: str, **extra_fields: Any
    ) -> StructuredLog:
        """創建結構化日誌"""
        import inspect
    ...
    def warning(self, message: str, **extra_fields: Any) -> None:
        ...
        log_entry = self._create_structured_log(
            LogLevel.WARNING, message, **extra_fields
        )
    ...
    def critical(self, message: str, **extra_fields: Any) -> None:
        """CRITICAL 級別日誌"""
        log_entry = self._create_structured_log(
            LogLevel.CRITICAL, message, **extra_fields
        )
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                metrics = PerformanceMetrics(
                    operation_name=name, duration_ms=duration_ms, success=True
                )
