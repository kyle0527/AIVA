...
class SchemaValidator:
    ...
    def validate_python_schemas(self, schema_dir: str = "services/aiva_common/schemas/generated") -> bool:
        """驗證Python生成的Schema"""
    ...
    def _analyze_python_schema(self, py_file: Path) -> bool:
        ...
        try:
            ...
            if spec and spec.loader:
                ...
                logger.info(f"  ✅ {py_file.name}: 找到 {len(schema_classes)} 個Schema類別")
                return True
    ...
    def _extract_pydantic_models(self, module) -> Dict[str, Any]:
        ...
        for attr_name in dir(module):
            ...
            if (hasattr(attr, '__mro__') and 
                any('BaseModel' in str(base) for base in attr.__mro__)):
                
                models[attr_name] = {
                    'fields': list(attr.model_fields.keys()) if hasattr(attr, 'model_fields') else [],
                    'class': attr
                }
        
        return models
    
    def validate_go_schemas(self, schema_file: str = "services/features/common/go/aiva_common_go/schemas/generated/schemas.go") -> bool:
        """驗證Go Schema檔案"""
    ...
    def check_cross_language_consistency(self) -> bool:
        ...
        total_python_classes = sum(len(classes) for classes in self.python_schemas.values())
