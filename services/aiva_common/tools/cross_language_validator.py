...
class CrossLanguageValidator:
    ...
    def __init__(self):
        ...
        self.generated_files = {
            'python': {
                'base_types': Path('services/aiva_common/schemas/generated/base_types.py'),
                'messaging': Path('services/aiva_common/schemas/generated/messaging.py'),
                ...
                'async_utils': Path('services/aiva_common/schemas/generated/async_utils.py'),
                'plugins': Path('services/aiva_common/schemas/generated/plugins.py'),
                'cli': Path('services/aiva_common/schemas/generated/cli.py'),
            },
            'go': {
                'all': Path('services/features/common/go/aiva_common_go/schemas/generated/schemas.go')
            },
            'rust': {
                'all': Path('services/scan/info_gatherer_rust/src/schemas/generated/mod.rs')
            }
        }
    ...
    def _validate_field_consistency(self, schema: SchemaDefinition, field: Any) -> None:
        ...
        for lang in ['python', 'go', 'rust']:
            ...
            # 如果映射失敗（返回原類型），記錄問題
            if mapped_type == source_type and source_type not in ['str', 'int', 'bool', 'float']:
                self._add_issue(
                    "warning",
                    "mismatch",
    ...
    def _generate_coverage_stats(self) -> Dict[str, Any]:
        ...
        for lang in ['python', 'go', 'rust']:
            lang_issues = [i for i in self.issues if i.languages and lang in i.languages]
            
            # 分類問題嚴重程度
            critical_issues = [i for i in lang_issues if i.severity in ['critical', 'error']]
        ...
        stats["field_type_distribution"] = dict(sorted(type_counts.items(), key=lambda x: x[1], reverse=True))
        
        return stats
    
    def _add_issue(self, severity: str, category: str, message: str, schema_name: str, 
                   field_name: Optional[str] = None, languages: Optional[List[str]] = None,
    ...
    def generate_report_file(self, report: ValidationReport, output_path: str = "cross_language_validation_report.json") -> None:
        """生成驗證報告文件"""
    ...
    def print_summary(self, report: ValidationReport) -> None:
        ...
        print(f"📊 總計: {report.total_schemas} 個 Schema, {report.total_fields} 個字段")
        ...
        for lang, stats in report.coverage_stats["language_coverage"].items():
            print(f"   {lang}: {stats['coverage_percentage']:.1f}% ({stats['issues_count']} 問題)")
