        ...
        try:
            ...
            if self.output_file:
                with open(self.output_file, "a", encoding="utf-8") as f:
                    f.write(
                        json.dumps(metrics_with_timestamp, ensure_ascii=False) + "\n"
                    )

            logger.debug(
                f"Exported metrics: {json.dumps(metrics_with_timestamp, indent=2)}"
            )
            return True
        ...
        with self._lock:
            ...
            return {
                ...
                "summary": {
                    ...
                    "failure_rate": (
                        (metrics.tasks_failed / total_tasks) if total_tasks > 0 else 0.0
                    ),
                    "retry_rate": (
                        (metrics.tasks_retried / total_tasks)
                        if total_tasks > 0
                        else 0.0
                    ),
...
def initialize_metrics(
    ...
    with _collector_lock:
        if _global_collector is not None:
            logger.warning(
                f"Metrics already initialized for {_global_collector.worker_id}"
            )
            return _global_collector
