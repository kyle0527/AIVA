...
class PluginManifest(BaseModel):
    ...
    config_schema: Optional[Dict[str, Any]] = Field(default=None, description="配置 Schema")
    min_aiva_version: str = Field(description="最低AIVA版本要求")
    max_aiva_version: Optional[str] = Field(default=None, description="最高AIVA版本要求")
    entry_point: str = Field(description="插件入口點")
    homepage: Optional[str] = Field(default=None, description="插件主頁")
    repository: Optional[str] = Field(default=None, description="源碼倉庫")
    ...
    def validate_permissions(cls, v: List[str]) -> List[str]:
        """驗證權限列表"""
        allowed_permissions = {
            "read_config", "write_config", "read_data", "write_data",
            "network_access", "file_system", "execute_commands", "admin_access"
        }
        for permission in v:
...
class PluginExecutionContext(BaseModel):
    ...
    timeout_seconds: int = Field(default=60, ge=1, le=600, description="執行超時時間(秒)")
...
class PluginExecutionResult(BaseModel):
    ...
    memory_usage_mb: Optional[float] = Field(default=None, ge=0, description="內存使用(MB)")
...
class PluginConfig(BaseModel):
    ...
    environment_variables: Dict[str, str] = Field(default_factory=dict, description="環境變數")
...
class PluginRegistry(BaseModel):
    ...
    registry_url: Optional[str] = Field(default=None, description="註冊表URL")
    last_sync: Optional[datetime] = Field(default=None, description="最後同步時間")
...
class PluginHealthCheck(BaseModel):
    ...
    last_check: datetime = Field(default_factory=lambda: datetime.now(UTC), description="最後檢查時間")
