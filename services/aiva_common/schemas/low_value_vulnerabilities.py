...
class VulnerabilityPattern(BaseModel):
    ...
    false_positive_indicators: list[str] = Field(
        default_factory=list, description="誤報指標"
    )
...
class ReflectedXSSBasic(XSSPattern):
    ...
    vulnerability_type: Literal[LowValueVulnerabilityType.REFLECTED_XSS_BASIC] = (
        LowValueVulnerabilityType.REFLECTED_XSS_BASIC
    )
...
class DOMXSSSimple(XSSPattern):
    ...
    vulnerability_type: Literal[LowValueVulnerabilityType.DOM_XSS_SIMPLE] = (
        LowValueVulnerabilityType.DOM_XSS_SIMPLE
    )
...
class CSRFMissingToken(CSRFPattern):
    ...
    vulnerability_type: Literal[LowValueVulnerabilityType.CSRF_MISSING_TOKEN] = (
        LowValueVulnerabilityType.CSRF_MISSING_TOKEN
    )
...
class CSRFJSONBypass(CSRFPattern):
    ...
    vulnerability_type: Literal[LowValueVulnerabilityType.CSRF_JSON_BYPASS] = (
        LowValueVulnerabilityType.CSRF_JSON_BYPASS
    )
...
class IDORSimpleID(IDORPattern):
    ...
    vulnerability_type: Literal[LowValueVulnerabilityType.IDOR_SIMPLE_ID] = (
        LowValueVulnerabilityType.IDOR_SIMPLE_ID
    )
...
class IDORUserData(IDORPattern):
    ...
    vulnerability_type: Literal[LowValueVulnerabilityType.IDOR_USER_DATA] = (
        LowValueVulnerabilityType.IDOR_USER_DATA
    )
...
class LowValueVulnerabilityTest(BaseModel):
    ...
    max_difficulty: VulnerabilityDifficulty = Field(
        default=VulnerabilityDifficulty.MEDIUM, description="最高難度"
    )
    ...
    # 優先級策略
    prioritize_by: Literal["success_rate", "bounty_amount", "discovery_time", "roi"] = (
        Field(default="roi", description="優先級策略")
    )
...
class LowValueVulnerabilityResult(BaseModel):
    ...
    report_draft: str | None = Field(default=None, description="報告草稿")
...
class BugBountyStrategy(BaseModel):
    ...
    # 資源分配
    low_value_allocation_percent: int = Field(
        default=80, ge=0, le=100, description="低價值漏洞資源分配百分比"
    )
    high_value_allocation_percent: int = Field(
        default=20, ge=0, le=100, description="高價值漏洞資源分配百分比"
    )
    ...
    max_hours_per_program: float = Field(
        default=2.0, description="每個程式最大測試時間（小時）"
    )
    ...
    # 優先級設定
    preferred_vulnerability_types: list[LowValueVulnerabilityType] = Field(
        description="偏好漏洞類型"
    )
    avoided_vulnerability_types: list[LowValueVulnerabilityType] = Field(
        default_factory=list, description="避免漏洞類型"
    )
...
class ROIAnalysis(BaseModel):
    ...
    # 趨勢分析
    roi_trend: Literal["improving", "stable", "declining"] = Field(
        description="ROI 趨勢"
    )
