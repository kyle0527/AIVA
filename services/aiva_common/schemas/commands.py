...
class AICommand(BaseModel):
    ...
    # 追蹤信息
    trace_id: str | None = Field(None, description="追蹤 ID，用於關聯相關命令")
    session_id: str | None = Field(None, description="會話 ID")
    parent_command_id: str | None = Field(None, description="父命令 ID（用於子命令）")
    ...
    callback_url: str | None = Field(None, description="完成後的回調 URL（可選，用於 HTTP 回調）")
    ...
    _callback_handler: CommandCallback | None = PrivateAttr(default=None)
    ...
    def get_callback(self) -> CommandCallback | None:
        """獲取回調處理器"""
...
class AICommandResult(BaseModel):
    ...
    started_at: datetime | None = Field(None, description="開始執行時間")
    completed_at: datetime | None = Field(None, description="完成時間")

    # 錯誤信息
    error: str | None = Field(None, description="錯誤信息（如果失敗）")
    error_code: str | None = Field(None, description="錯誤代碼")
    error_details: dict[str, Any] | None = Field(None, description="詳細錯誤信息")
...
class CommandContext(BaseModel):
    ...
    # 追蹤信息
    trace_id: str | None = Field(None, description="追蹤 ID，用於關聯相關操作")

    # 身份信息
    user_id: str | None = Field(None, description="用戶 ID")
    session_id: str | None = Field(None, description="會話 ID")
    ...
    # 資源限制
    max_memory_mb: int | None = Field(None, description="最大內存限制（MB）")
    max_cpu_percent: int | None = Field(None, ge=1, le=100, description="最大 CPU 使用率（%）")
...
class CLICommand(BaseModel):
    ...
    這是 AI 向 CLI 執行器下達命令的標準格式，實現完全解耦。
    AI 只產出 CLICommand 物件，不直接調用任何具體實現。

    設計原則（CLI 參數包驅動架構）:
    - AI 只產出結構化的 CLI 參數包
    - 執行器將參數包轉換為實際的 CLI 命令
    - 完全解耦 AI 決策層和執行層
    - 天然沙盒：AI 無法執行任意代碼
    ...
    使用示例:
        ...
        # 執行器轉換為實際命令
        args = cli_cmd.to_cli_args()
        # ["python", "-m", "services.core.aiva_core.core_capabilities.cli.aiva_cli",
        #  "flow8", "--target", "https://example.com", "--intensity", "0.8",
        #  "--mode", "stealth", "--concurrency", "5"]
    ...
    flow_id: str = Field(
        ...,
        description="對應 latest_classification.json 中的 flow id 或 name",
        examples=["flow_8", "nmap_scan", "sqli_detection"]
    )

    target: str = Field(
        ...,
        description="攻擊或掃描目標（URL、IP、域名等）",
        examples=["https://example.com", "192.168.1.1", "example.com:8080"]
    )

    flags: dict[str, Any] = Field(
        default_factory=dict,
        description="CLI 的額外參數，如 intensity, mode, concurrency 等"
    )
    ...
    command_id: str | None = Field(
        None,
        description="命令 ID，用於追蹤（自動生成）"
    )

    trace_id: str | None = Field(
        None,
        description="追蹤 ID，用於關聯相關命令"
    )

    # 元數據
    metadata: dict[str, Any] = Field(
        default_factory=dict,
        description="額外的元數據（AI 決策推理、策略信息等）"
    )
    ...
    def to_cli_args(self) -> list[str]:
        ...
        Returns:
            List[str]: 可直接用於 subprocess.run() 的參數列表

        Examples:
            >>> cmd = CLICommand(flow_id="flow_8", target="example.com",
            ...                  flags={"intensity": 0.8})
            >>> cmd.to_cli_args()
            ['python', '-m', 'services.core.aiva_core.core_capabilities.cli.aiva_cli',
             'flow8', '--target', 'example.com', '--intensity', '0.8']
        """
        args = [
            "python",
            "-m",
        ...
        # 提取 flow 編號（流程名稱標準化）
        # "flow_8" → "flow8", "nmap-scan" → "nmapscan"
        ...
        for key, value in self.flags.items():
            # 布林值處理：True 只添加 flag；False 跳過整個 flag
            if value is False:
                pass  # 跳過 False 值的 flag
            elif value is True:
                args.append(f"--{key}")  # 只添加 flag
            else:
                args.append(f"--{key}")
    ...
    def to_shell_command(self) -> str:
        ...
        Returns:
            str: 可用於日誌記錄或手動執行的命令字串

        Examples:
            >>> cmd = CLICommand(flow_id="flow_8", target="example.com")
            >>> cmd.to_shell_command()
            'python -m services.core.aiva_core.core_capabilities.cli.aiva_cli flow8 --target example.com'
        """
    ...
    def from_flow_info(
        ...
        **kwargs
        ...
        """從 Flow 信息創建 CLI 命令（工廠方法）

        Args:
            flow_id: Flow ID
            target: 目標地址
            intent: AI 意圖（scan, exploit, analyze 等）
            capability_type: 能力類型（nmap, sqli, xss 等）
            **kwargs: 其他 flags 參數

        Returns:
            CLICommand: 構建好的 CLI 命令實例
        """
        metadata = {}
        if intent:
