...
# ==================== 常數定義 ====================
# 遵循PEP-8標準，定義重複使用的字串常數
DEPRECATED_FIELD_DESC = "是否已棄用"
SERVER_LIST_DESC = "伺服器列表"
...
class OpenAPISchema(BaseModel):
    ...
    deprecated: bool | None = Field(default=None, description=DEPRECATED_FIELD_DESC)
...
class OpenAPIOperation(BaseModel):
    ...
    servers: list[OpenAPIServer] | None = Field(
        default=None, description=SERVER_LIST_DESC
    )
...
class GraphQLFieldDefinition(BaseModel):
    ...
    is_deprecated: bool = Field(default=False, description=DEPRECATED_FIELD_DESC)
