...
class CLIParameter(BaseModel):
    ...
    default_value: Optional[Union[str, int, float, bool]] = Field(default=None, description="默認值")
    choices: Optional[List[str]] = Field(default=None, description="可選值列表")
    ...
    help_text: Optional[str] = Field(default=None, description="幫助文本")
    
    @field_validator("type")
    @classmethod
    def validate_type(cls, v: str) -> str:
        """驗證參數類型"""
        allowed_types = {"string", "integer", "float", "boolean", "choice", "file", "directory"}
...
class CLICommand(BaseModel):
    ...
    parameters: List[CLIParameter] = Field(default_factory=list, description="命令參數列表")
    ...
    max_args: Optional[int] = Field(default=None, ge=0, description="最多參數數量")
    ...
    def validate_category(cls, v: str) -> str:
        """驗證命令分類"""
        allowed_categories = {
            "general", "scan", "security", "analysis", "reporting", 
            "config", "admin", "debug", "plugin", "utility"
        }
...
class CLIExecutionResult(BaseModel):
    ...
    start_time: datetime = Field(default_factory=lambda: datetime.now(UTC), description="開始時間")
    end_time: Optional[datetime] = Field(default=None, description="結束時間")
    user_id: Optional[str] = Field(default=None, description="執行用戶ID")
    session_id: Optional[str] = Field(default=None, description="會話ID")
    working_directory: Optional[str] = Field(default=None, description="工作目錄")
    environment: Dict[str, str] = Field(default_factory=dict, description="環境變數")
    resource_usage: Dict[str, Any] = Field(default_factory=dict, description="資源使用情況")
...
class CLISession(BaseModel):
    ...
    start_time: datetime = Field(default_factory=lambda: datetime.now(UTC), description="開始時間")
    end_time: Optional[datetime] = Field(default=None, description="結束時間")
    commands_executed: List[str] = Field(default_factory=list, description="已執行命令列表")
    current_directory: str = Field(default="/", description="當前目錄")
    environment: Dict[str, str] = Field(default_factory=dict, description="會話環境變數")
    settings: Dict[str, Any] = Field(default_factory=dict, description="會話設置")
    history: List[CLIExecutionResult] = Field(default_factory=list, description="命令歷史")
    status: str = Field(default="active", description="會話狀態")
    last_activity: datetime = Field(default_factory=lambda: datetime.now(UTC), description="最後活動時間")
...
class CLIConfiguration(BaseModel):
    ...
    global_settings: Dict[str, Any] = Field(default_factory=dict, description="全局設置")
    command_settings: Dict[str, Dict[str, Any]] = Field(default_factory=dict, description="命令特定設置")
    aliases: Dict[str, str] = Field(default_factory=dict, description="命令別名映射")
    default_parameters: Dict[str, Any] = Field(default_factory=dict, description="默認參數")
    security_settings: Dict[str, Any] = Field(default_factory=dict, description="安全設置")
    ui_settings: Dict[str, Any] = Field(default_factory=dict, description="界面設置")
    logging_settings: Dict[str, Any] = Field(default_factory=dict, description="日誌設置")
    created_at: datetime = Field(default_factory=lambda: datetime.now(UTC), description="創建時間")
    updated_at: datetime = Field(default_factory=lambda: datetime.now(UTC), description="更新時間")
...
class CLIMetrics(BaseModel):
    ...
    average_execution_time_ms: float = Field(default=0, ge=0, description="平均執行時間(毫秒)")
    most_used_commands: List[Dict[str, Any]] = Field(default_factory=list, description="最常用命令")
    error_rates: Dict[str, float] = Field(default_factory=dict, description="錯誤率統計")
    user_activity: Dict[str, int] = Field(default_factory=dict, description="用戶活動統計")
    resource_usage: Dict[str, Any] = Field(default_factory=dict, description="資源使用統計")
    start_time: datetime = Field(default_factory=lambda: datetime.now(UTC), description="統計開始時間")
    end_time: datetime = Field(default_factory=lambda: datetime.now(UTC), description="統計結束時間")
