...
class ResourceLimits(BaseModel):
    ...
    max_memory_mb: Optional[int] = Field(default=None, ge=1, description="最大內存限制(MB)")
    max_cpu_percent: Optional[float] = Field(default=None, ge=0.1, le=100.0, description="最大CPU使用率(%)")
    max_execution_time: Optional[int] = Field(default=None, ge=1, description="最大執行時間(秒)")
    max_concurrent_tasks: int = Field(default=10, ge=1, le=100, description="最大並發任務數")
...
class AsyncTaskConfig(BaseModel):
    ...
    retry_config: RetryConfig = Field(default_factory=RetryConfig, description="重試配置")
    priority: int = Field(default=5, ge=1, le=10, description="任務優先級")
    resource_limits: ResourceLimits = Field(default_factory=ResourceLimits, description="資源限制")
...
class AsyncBatchConfig(BaseModel):
    ...
    batch_timeout_seconds: int = Field(default=3600, ge=1, description="批次超時時間(秒)")
...
class AsyncBatchResult(BaseModel):
    ...
    task_results: List[AsyncTaskResult] = Field(default_factory=list, description="任務結果列表")
    batch_status: str = Field(description="批次狀態")
    start_time: datetime = Field(default_factory=lambda: datetime.now(UTC), description="開始時間")
    end_time: Optional[datetime] = Field(default=None, description="結束時間")
    total_execution_time_ms: float = Field(default=0, ge=0, description="總執行時間(毫秒)")
