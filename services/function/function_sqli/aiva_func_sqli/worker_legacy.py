...
# SqliExecutionTelemetry 已從 worker.py 導入


class SqliDetectionOrchestrator:
    ...
    async def execute_detection(
        self, context: SqliDetectionContext, client: httpx.AsyncClient
    ) -> SqliDetectionContext:
        """執行所有已註冊的檢測引擎"""
    ...
    async def _build_finding(
        ...
        return FindingPayload(
            finding_id=new_id("finding"),  # 修正：使用正確的欄位名
            task_id=task.task_id,  # 新增：從 task 傳遞 task_id
            scan_id=task.scan_id,  # 新增：從 task 傳遞 scan_id
            status="new",  # 新增：設定初始狀態
...
class SqliWorkerService:
    ...
    def __init__(
        self,
        orchestrator: SqliDetectionOrchestrator | None = None,
        publisher: SqliResultBinderPublisher | None = None,
        config: SqliEngineConfig | None = None,
    ):
        self.orchestrator = orchestrator or SqliDetectionOrchestrator(config)
    ...
    async def process_task(
        ...
        context = SqliDetectionContext(task=task, config=self.config)
