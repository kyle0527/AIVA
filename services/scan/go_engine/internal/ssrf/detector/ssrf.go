...
func (d *SSRFDetector) Scan(ctx context.Context, targets []string, config *common.ScannerConfig) ([]common.Asset, error) {
	log.Printf("[INFO] 開始 SSRF 掃描: targets=%d, concurrency=%d\n", len(targets), config.Concurrency)
	...
	for _, target := range targets {
		...
		if err != nil {
			log.Printf("[WARN] 目標掃描失敗: target=%s, error=%v\n", target, err)
			continue
		}
	...
	// 🔍 診斷: 輸出請求統計信息
	log.Printf("[INFO] SSRF 掃描完成: assets=%d, attempted=%d, sent=%d, failed=%d, read=%d\n",
		len(allAssets), d.requestsAttempted.Load(), d.requestsSent.Load(),
		d.requestsFailed.Load(), d.responsesRead.Load())
	...
	for _, param := range ssrfParams {
		for _, payload := range testPayloads {
			...
			if asset := d.testSSRF(ctx, testURL, target, param, payload); asset != nil {
				...
				log.Printf("[WARN] 🚨 檢測到 SSRF 漏洞 - param: %s, payload_type: %s", param, payload.name)
			}
		}
	}
