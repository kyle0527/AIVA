...
func (m *Monitor) StartHTTP(port int) {
	...
	go func() {
		log.Printf("[INFO] OOB Monitor listening on port %d\n", port)
		if err := http.ListenAndServe(fmt.Sprintf(":%d", port), nil); err != nil {
			log.Printf("[ERROR] Failed to start OOB monitor: %v\n", err)
		}
	}()
}
...
func (m *Monitor) recordInteraction(token, ip, interactionType string) {
	...
	log.Printf("[WARN] [OOB] Detected interaction for token %s from %s\n", $2)
