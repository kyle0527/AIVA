...
func (v *Verifier) Verify(ctx context.Context, candidate Candidate) ([]VerificationResult, error) {
	...
	log.Printf("[INFO] [SSRF Verifier] Starting verification for %s (param: %s)\n", url, param)
	...
	for _, payload := range v.payloads {
		...
		go func(p string) {
			...
			if success {
				...
				log.Printf("[WARN] [SSRF Verifier] VULNERABLE: %s with payload %s\n", url, payload)
			}
		}(payload)
	}
	...
	log.Printf("[INFO] [SSRF Verifier] Completed verification for %s, found %d vulnerabilities\n", $2))
	return results, nil
}
