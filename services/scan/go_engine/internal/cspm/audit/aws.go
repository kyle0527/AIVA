...
//
// ⚠️ 注意: 此模組需要 AWS SDK 外部依賴
// - github.com/aws/aws-sdk-go-v2/config
// - github.com/aws/aws-sdk-go-v2/service/s3
//
// 這是可選功能模組，如不需要可從編譯中排除
// 編譯標籤: // +build aws
// =====================================================================
...
func (a *AWSAuditor) AuditS3Buckets() ([]string, error) {
	...
	log.Printf("[INFO] [AWS Auditor] Found %d S3 buckets\n", len(output.Buckets))
	...
	for _, bucket := range output.Buckets {
		...
		if err != nil {
			log.Printf("[WARN] Failed to get ACL for bucket %s: %v\n", name, err)
			continue
		}

		if isPublicACL(acl) {
			riskBuckets = append(riskBuckets, name)
			log.Printf("[WARN] [AWS Auditor] RISK: Bucket %s has public ACL\n", name)
			continue
		}
		...
		if err != nil {
			...
			riskBuckets = append(riskBuckets, name)
			log.Printf("[WARN] [AWS Auditor] RISK: Bucket %s allows public access\n", name)
		}
	}

	log.Printf("[INFO] [AWS Auditor] Audit completed, found %d risk buckets\n", len(riskBuckets))
...
func (a *AWSAuditor) AuditIAMUsers() ([]string, error) {
	...
	// 目前返回空結果，不影響其他模組運行
	log.Println("[INFO] [AWS Auditor] IAM audit not yet implemented")
	return nil, nil
}
...
func (a *AWSAuditor) AuditSecurityGroups() ([]string, error) {
	...
	// 目前返回空結果，不影響其他模組運行
	log.Println("[INFO] [AWS Auditor] Security Group audit not yet implemented")
	return nil, nil
}
...
func (a *AWSAuditor) AuditCloudTrail() ([]string, error) {
	...
	// 目前返回空結果，不影響其他模組運行
	log.Println("[INFO] [AWS Auditor] CloudTrail audit not yet implemented")
	return nil, nil
}
...
func (a *AWSAuditor) AuditKMSKeys() ([]string, error) {
	...
	// 目前返回空結果，不影響其他模組運行
	log.Println("[INFO] [AWS Auditor] KMS audit not yet implemented")
	return nil, nil
}
...
func (a *AWSAuditor) RunFullAudit() (map[string][]string, error) {
	...
	if s3Risks, err := a.AuditS3Buckets(); err == nil {
		...
		log.Printf("[ERROR] S3 audit failed: %v\n", err)
	}
	...
	log.Printf("[INFO] [AWS Auditor] Full audit completed, scanned %d service categories\n", len(results))
	return results, nil
}
