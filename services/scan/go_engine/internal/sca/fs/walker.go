...
func ScanDirectory(rootPath string) (*WalkResult, error) {
	...
	log.Printf("[INFO] [FS Walker] Starting scan of directory: %s\n", $2)

	err := filepath.Walk(rootPath, func(path string, info os.FileInfo, err error) error {
		if err != nil {
			// 記錄錯誤但繼續遍歷
			log.Printf("[WARN] [FS Walker] Error accessing path %s: %v\n", $2)
		...
		if !info.IsDir() {
			...
			if fileType != "" {
				log.Printf("[INFO] [FS Walker] Found dependency file: %s (type: %s)\n", $2)
	...
	log.Printf("[INFO] [FS Walker] Scan completed, found %d dependency files\n", $2))
	return result, nil
}
