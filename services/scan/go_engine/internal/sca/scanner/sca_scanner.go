...
func NewSCAScanner(logger *log.Logger) *SCAScanner {
	return &SCAScanner{
		logger: logger,
	}
}
...
// Scan 執行 SCA 掃描
func (s *SCAScanner) Scan(ctx context.Context, task models.ScanTaskPayload) ([]models.FindingPayload, error) {
	s.logger.Printf("[INFO] Starting SCA scan: target_url=%s\n", task.Target.URL)
	...
	s.logger.Printf("[INFO] Detected package files: count=%d\n", len(packageFiles))
	// 3. 使用 OSV-Scanner 掃描
...
func (s *SCAScanner) createFinding(
	...
	// 建立最終的 models.FindingPayload - 注意字段名稱和類型（駝峰命名，指針類型）
