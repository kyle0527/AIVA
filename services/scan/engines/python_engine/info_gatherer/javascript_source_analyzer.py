...
from services.aiva_common.enums import AlertSeverity
...
class AnalysisResult:
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        return {
            ...
            "critical_issues": len(
                [s for s in self.sinks if s.severity == AlertSeverity.ERROR]
            )
            + len([p for p in self.patterns if p.severity == AlertSeverity.ERROR]),
            "high_issues": len([s for s in self.sinks if s.severity == AlertSeverity.ERROR])
            + len([p for p in self.patterns if p.severity == AlertSeverity.ERROR]),
...
class JavaScriptSourceAnalyzer:
    ...
    def _analyze_dataflow(self, lines: list[str], result: AnalysisResult) -> None:
        ...
        for sink in result.sinks:
            ...
            if nearby_sources:
                ...
                # 提升嚴重程度
                if sink.severity == AlertSeverity.ERROR:
                    sink.severity = AlertSeverity.ERROR
    ...
    def get_high_risk_issues(
        ...
        for sink in result.sinks:
            if sink.severity in [AlertSeverity.ERROR, AlertSeverity.ERROR]:
                high_risk.append(sink)

        for pattern in result.patterns:
            if pattern.severity in [AlertSeverity.ERROR, AlertSeverity.ERROR]:
