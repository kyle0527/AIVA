...
def _should_run_ssrf_scan(req: Phase1StartPayload) -> bool:
    ...
    Phase 1 模式：檢查 Phase0 結果中是否存在 Candidate URLs
    """
    # 從 Phase0 結果提取 Candidate URLs
    if req.phase0_result:
        endpoints = req.phase0_result.get("endpoints", [])
        # 檢查是否有帶參數的端點
        candidate_urls = [
            ep for ep in endpoints 
            if ep.get("params")  # 只有帶參數的端點才是 SSRF Candidate
        ]
