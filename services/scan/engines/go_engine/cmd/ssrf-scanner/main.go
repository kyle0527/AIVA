...
func main() {
	...
	logger.Info("SSRF Scanner started",
	...
	logger.Info("SSRF Scanner completed",
		zap.String("scan_id", request.ScanID),
...
func executeScan(
	...
	for i, target := range request.Targets {
		...
		if err := pool.Submit(task); err != nil {
			logger.Error("Failed to submit task", zap.Error(err), zap.String("target", target))
		...
		case taskResult, ok := <-pool.Results():
			...
			if taskResult.Error != nil {
				...
				logger.Warn("Task failed",
					zap.String("target", taskResult.Target),
