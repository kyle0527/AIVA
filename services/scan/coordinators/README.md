...
- [🎉 重構完成總結](#-重構完成總結)
  - [❌ 驗證失敗 (v2.1 - 2025年11月22日)](#-驗證失敗-v21---2025年11月22日)
  - [🏗️ 適配器模式實現](#-適配器模式實現)
- [📋 目錄](#-目錄)
  - [核心組件](#核心組件)
  - [技術文檔](#技術文檔)
...
- [🏗️ 架構設計](#-架構設計)
  - [設計原則](#設計原則)
  - [架構層次](#架構層次)
- [🎨 協調器決策與引擎調用圖](#-協調器決策與引擎調用圖)
  - [📊 AI 決策流程圖](#-ai-決策流程圖)
  - [🔧 四種引擎調用方式：數據合約跨語言通信](#-四種引擎調用方式數據合約跨語言通信)
  - [🔄 協調器內部運作機制詳解](#-協調器內部運作機制詳解)
  - [🔄 協調器內部運作流程圖](#-協調器內部運作流程圖)
  - [📊 數據合約在各階段的形態](#-數據合約在各階段的形態)
  - [🔄 完整調用流程序列圖](#-完整調用流程序列圖)
  - [💡 實際調用代碼示例](#-實際調用代碼示例)
- [🎯 協調器完整操作指南](#-協調器完整操作指南)
  - [如何使用協調器切換引擎和模式](#如何使用協調器切換引擎和模式)
  - [引擎和模式的自動映射](#引擎和模式的自動映射)
  - [手動指定引擎配置](#手動指定引擎配置)
  - [實際使用場景示例](#實際使用場景示例)
  - [協調器內部決策邏輯](#協調器內部決策邏輯)
- [📦 核心模組](#-核心模組)
  - [檔案結構與功用說明](#檔案結構與功用說明)
  - [1. `__init__.py` - 模組初始化與導出](#1-__init__py---模組初始化與導出)
  - [2. `scan_models.py` - 數據模型定義（174 行）](#2-scan_modelspy---數據模型定義174-行)
  - [3. `multi_engine_coordinator.py` - 多引擎協調器（1536 行）⭐ 核心](#3-multi_engine_coordinatorpy---多引擎協調器1536-行-核心)
  - [4. `unified_scan_engine.py` - 統一掃描引擎（302 行）](#4-unified_scan_enginepy---統一掃描引擎302-行)
  - [5. `target_generators/` - 測試目標生成器](#5-target_generators---測試目標生成器)
  - [6. `start_scan_live.ps1` - PowerShell 快速啟動腳本（203 行）](#6-start_scan_liveps1---powershell-快速啟動腳本203-行)
  - [7. `docker-compose.scan.yml` - Docker Compose 配置](#7-docker-composescanyml---docker-compose-配置)
  - [總結：各腳本的角色定位](#總結各腳本的角色定位)
  - [MultiEngineCoordinator - 多引擎協調器](#multienginecoordinator---多引擎協調器)
  - [UnifiedScanEngine - 統一掃描引擎](#unifiedscanengine---統一掃描引擎)
  - [ScanModels - 數據模型](#scanmodels---數據模型)
  - [Target Generators - 目標生成器](#target-generators---目標生成器)
- [🔄 掃描流程](#-掃描流程)
  - [完整掃描流程（4 階段）](#完整掃描流程4-階段)
  - [Phase 0: Rust 快速發現](#phase-0-rust-快速發現)
  - [Phase 1: AI 決策編排](#phase-1-ai-決策編排)
  - [Phase 2: 三引擎並行執行](#phase-2-三引擎並行執行)
  - [Phase 3: 結果聚合與分析](#phase-3-結果聚合與分析)
- [🎯 使用方式](#-使用方式)
  - [快速開始](#快速開始)
  - [引擎選擇建議](#引擎選擇建議)
- [📊 實際狀態](#-實際狀態)
