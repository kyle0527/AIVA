...
class BlindXssListenerValidator:
    ...
    依照 aiva_common v2.0 規範，必須配置 OAST_SERVICE_URL 環境變量。
    """
    ...
    def __init__(self, store: BlindCallbackStore | None = None) -> None:
        if store is None:
            ...
            if not base_url:
                raise RuntimeError(
                    "OAST_SERVICE_URL 環境變量未設置。"
                    "Blind XSS 檢測需要 OAST 服務支持。"
                    "請設置 OAST_SERVICE_URL 環境變量，例如: export OAST_SERVICE_URL=http://localhost:8083"
                )
            store = OastHttpCallbackStore(base_url=base_url)
        self._store = store
