...
async def _perform_test(payload: dict) -> list:
    ...
    try:
        if test_type == "price_manipulation":
            # 價格操控測試
            logger.info(f"Running price manipulation test on {target_url}")
            tester = PriceManipulationTester(target_url, authorization_token)
            
            endpoint = payload.get("endpoint", "/api/checkout")
            test_results = await tester.run_all_tests(endpoint)
            ...
            for result in test_results:
                finding = mk_finding(
                    ...
                    description=f"Price manipulation vulnerability: {result.get('evidence', '')}",
                    cwe_id="CWE-840",
                    remediation="Implement server-side price validation and authorization checks",
                    task_id=task_id,
                    scan_id=scan_id,
                    target=target_url,
                    evidence=result
            ...
            # 競態條件測試
            logger.info(f"Running race condition test on {target_url}")
            tester = RaceConditionTester(target_url, authorization_token)
            
            test_endpoints = payload.get("test_endpoints")
            test_results = await tester.run_all_tests(test_endpoints)
            ...
            for result in test_results:
                finding = mk_finding(
                    ...
                    severity=result["severity"],
                    confidence=result["confidence"],
                    description=f"Race condition vulnerability: {result.get('evidence', '')}",
                    cwe_id="CWE-362",
            ...
            # 工作流程繞過測試
            logger.info(f"Running workflow bypass test on {target_url}")
            tester = WorkflowBypassTester(target_url, authorization_token)
            
            test_config = payload.get("test_config")
            test_results = await tester.run_all_tests(test_config)
            ...
            for result in test_results:
                finding = mk_finding(
                    ...
                    severity=result["severity"],
                    confidence=result["confidence"],
                    description=f"Workflow bypass vulnerability: {result.get('evidence', '')}",
                    cwe_id="CWE-841",
                    remediation="Enforce proper workflow state validation and access controls",
                    task_id=task_id,
                    scan_id=scan_id,
                    target=target_url,
                    evidence=result
            ...
            logger.warning(f"Unknown bizlogic test type: {test_type}")
    
    except Exception as e:
        logger.exception(f"Business logic test error: {e}")
