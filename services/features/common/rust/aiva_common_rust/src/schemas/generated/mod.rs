...
impl Vulnerability {
    ...
    pub fn validate(&self) -> Result<(), String> {
        ...
        if self.severity.is_empty() {
            return Err("severity cannot be empty".to_string());
        }
        if self.confidence.is_empty() {
            return Err("confidence cannot be empty".to_string());
        }
        Ok(())
    }
}
...
impl FindingPayload {
    ...
    pub fn validate(&self) -> Result<(), String> {
        ...
        if self.scan_id.is_empty() {
            return Err("scan_id cannot be empty".to_string());
        }
...
pub struct FunctionTaskTarget {
    ...
    /// 表單資料
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub form_data: Option<std::collections::HashMap<String, serde_json::Value>>,
    /// JSON資料
    #[serde(default, skip_serializing_if = "Option::is_none")]
