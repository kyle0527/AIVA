...
class SensitiveInfoDetector:
    ...
    - 瑼Ｘ葫?脫???霅?(AWS, GCP, Azure)
    - 瑼Ｘ葫?豢?摨恍?摮泵銝?    - 瑼Ｘ葫?折頝臬??頂蝯曹縑??    - 瑼Ｘ葫 debug 靽⊥?隤文?璉?    - 瑼Ｘ葫 HTML 閮駁?銝剔???靽⊥
    ...
    雿輻蝭?:
        ...
        # 瑼Ｘ葫?踵???        result = detector.detect_in_headers(headers, url="https://example.com")
    ...
    def detect_in_html(self, html_content: str, url: str = "") -> DetectionResult:
        ...
        # 瑼Ｘ葫 JavaScript 隞?Ⅳ憛?        result.matches.extend(self._detect_script_blocks(html_content))
    ...
    def detect_in_javascript(self, js_code: str, url: str = "") -> DetectionResult:
        """
        瑼Ｘ葫 JavaScript 隞?Ⅳ銝剔???靽⊥

        Args:
            js_code: JavaScript 隞?Ⅳ
            url: ?單 URL
    ...
    def detect_in_response(
        ...
        Args:
            response_body: ?踵?擃?            headers: ?踵??哨??舫嚗?            url: 隢? URL
    ...
    def _detect_html_comments(self, html: str) -> list[SensitiveMatch]:
        ...
        for comment_match in re.finditer(comment_pattern, html, re.DOTALL):
            ...
            for match in self._detect_in_text(comment_text, Location.HTML_COMMENT):
                # ?湔?湧?蝔漲嚗酉?葉?援?脫?湧?嚗?                if match.severity == AlertSeverity.NOTICE:
