...
class DetectionDemo:
    ...
    async def simulate_sqli_detection(self) -> dict[str, Any]:
        ...
        for engine, result in engines_results.items():
            print(f"  ✅ {engine:12} - Found {result['vulnerabilities_found']} vulns (Confidence: {result.get('confidence', 'N/A')}%)")
        ...
        print(f"  🛡️  WAF bypass: {'Success' if summary['waf_bypass_success'] else 'Failed'}")
    ...
    async def simulate_ssrf_detection(self) -> dict[str, Any]:
        ...
        detection_results = {
            "cloud_metadata": {
                ...
                "gcp_metadata": {"vulnerable": False, "protection": "Metadata concealment"},
                "azure_imds": {"vulnerable": True, "endpoints": ["/metadata/instance"], "risk": "Medium"}
            },
            "internal_services": {
                "redis": {"port": 6379, "accessible": True, "data_exposure": "Session data"},
                "elasticsearch": {"port": 9200, "accessible": True, "indices": ["logs", "users"]},
                "kubernetes_api": {"port": 8080, "accessible": False, "protection": "Network policy"}
            },
            "bypass_techniques": {
        ...
        print(f"  📊 SSRF Summary: {summary['cloud_vulns_found']} cloud + {summary['internal_services_exposed']} internal services")
    ...
    async def simulate_xss_detection(self) -> dict[str, Any]:
        ...
        detection_results = {
            ...
            "framework_specific": {
                "angular": {"template_injection": True, "sanitizer_bypass": "{{constructor.constructor('alert(1)')()}}"},
        ...
        print(f"    ✅ Payloads: {reflected['successful_payloads']}/{reflected['basic_payloads']} successful")
        ...
        for framework, result in detection_results["framework_specific"].items():
            ...
            print(f"    {'🚨' if vulns > 0 else '✅'} {framework}: {vulns} vulnerabilities")

        print("  🛡️  CSP Bypass Analysis:")
        csp = detection_results["csp_bypass"]
        print(f"    🔍 Policy: {csp['policy']}")
        print(f"    ⚡ Bypasses: {csp['bypasses_found']} techniques, {csp['success_rate']} success")
        ...
        print(f"  📊 XSS Summary: {summary['total_xss_vulns']} total vulnerabilities across all types")
    ...
    async def simulate_idor_detection(self) -> dict[str, Any]:
        ...
        detection_results = {
            "intelligent_analysis": {
                "id_patterns_learned": ["UUID v4", "Sequential numeric", "Base64 encoded", "Hash-based"],
                "ml_predictions": 147,
                "successful_predictions": 89,
                "prediction_accuracy": "60.5%"
            },
            "api_testing": {
                "rest_endpoints": {"tested": 45, "vulnerable": 12, "bypasses": ["parameter_pollution", "http_verb_tampering"]},
                "graphql": {"queries_tested": 23, "vulnerable": 5, "info_disclosure": True},
                "grpc": {"services_tested": 8, "vulnerable": 2, "auth_bypass": True},
                "websocket": {"endpoints": 3, "message_tampering": True}
            },
            "multi_tenant": {
                "organization_isolation": {"tested": True, "breached": 3, "data_exposure": "Customer records"},
                "user_isolation": {"tested": True, "breached": 7, "privilege_escalation": True},
                "role_isolation": {"tested": True, "breached": 2, "admin_access": True}
            },
            "temporal_testing": {
                "session_fixation": True,
                "race_conditions": 2,
                "time_based_enumeration": {"successful": True, "pattern": "Incremental ID with timestamp"}
            },
        ...
        print(f"    📈 ML Predictions: {intel['successful_predictions']}/{intel['ml_predictions']} ({intel['prediction_accuracy']})")
        ...
        for api_type, result in detection_results["api_testing"].items():
            if isinstance(result, dict) and "vulnerable" in result:
                print(f"    {'⚠️' if result['vulnerable'] > 0 else '✅'} {api_type}: {result['vulnerable']}/{result['tested']} endpoints vulnerable")
        ...
        print(f"  📊 IDOR Summary: {summary['total_idor_vulns']} vulnerabilities with AI assistance")
    ...
    async def run_comprehensive_demo(self):
        ...
        total_vulns = (
            ...
            ssrf_results.get('cloud_vulns_found', 0) + ssrf_results.get('internal_services_exposed', 0) +
        ...
        print(f"   • SQLi:  {sqli_results.get('total_vulnerabilities', 0)} vulnerabilities")
        print(f"   • SSRF:  {ssrf_results.get('cloud_vulns_found', 0) + ssrf_results.get('internal_services_exposed', 0)} exposures")
