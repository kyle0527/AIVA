...
# 讀取功能分析結果
with open('C:\\D\\fold7\\AIVA-git\\_out\\script_functionality_report.json', 'r', encoding='utf-8') as f:
    data = json.load(f)

# 讀取樹狀圖
tree_path = 'C:\\D\\fold7\\AIVA-git\\_out\\tree_ultimate_chinese_20251019_082355.txt'
...
for status, symbol in [
    ...
    for item in data.get(status, []):
        path = item['file'].replace('C:\\D\\fold7\\AIVA-git\\', '').replace('\\', '/')
