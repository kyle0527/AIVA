...
class AIPlayground:
    ...
    def __init__(self, possibilities_file: Path, experience_file: Path | None = None):
        ...
        # 載入已有經驗
        self.experience_file = experience_file or Path(
            "_out/ai_playground/experience.json"
        )
        ...
        logger.info(
            f"  未探索: {len(self.all_combinations) - len(self.experience.get('tried_combinations', []))}"
        )
    ...
    def random_try(self, n: int = 1) -> list[dict[str, Any]]:
        ...
        for i in range(n):
            ...
            logger.info(
                f"  [{i + 1}/{n}] {status} {result['pattern']} - {result['mode']}"
            )
    ...
    def curious_explore(self, n: int = 10) -> list[dict[str, Any]]:
        """好奇心驅動探索 - 優先嘗試未知組合.
        ...
        for i, combo in enumerate(selected):
            ...
            logger.info(
                f"  [{i + 1}/{n}] {status}{is_new} {result['pattern']} - {result['complexity']}"
            )
    ...
    def smart_explore(self, n: int = 10) -> list[dict[str, Any]]:
        ...
        for pattern, data in successful_patterns.items():
            success_rate = data.get("success_count", 0) / max(
                data.get("total_count", 1), 1
            )
        ...
        sorted_combos = sorted(
            untried, key=lambda c: pattern_scores.get(c["pattern"], 0), reverse=True
        )
        ...
        for i, combo in enumerate(selected):
            ...
            logger.info(
                f"  [{i + 1}/{n}] {status} {result['pattern']} (信心度: {confidence:.0f}%)"
            )
    ...
    def try_specific(
        self, mode: str | None = None, ports: list[int] | None = None
    ) -> dict[str, Any]:
        """嘗試特定組合.
        ...
        matching = [
            ...
            and (ports is None or c["ports"] == ports)
        ]
    ...
    def _try_combination(self, combo: dict[str, Any]) -> dict[str, Any]:
        ...
        if success:
            ...
            if len(self.experience["successful_patterns"][pattern]["examples"]) < 3:
                self.experience["successful_patterns"][pattern]["examples"].append(
                    command
                )
        else:
    ...
    def _print_session_stats(self, results: list[dict[str, Any]]) -> None:
        ...
        logger.info("本次統計:")
        logger.info(
            f"  成功: {success_count}/{len(results)} ({success_count / len(results) * 100:.1f}%)"
        )
    ...
    def show_stats(self) -> None:
        ...
        if self.curiosity_stats["total_tries"] > 0:
            success_rate = (
                self.curiosity_stats["successful_tries"]
                / self.curiosity_stats["total_tries"]
                * 100
            )
        ...
        logger.info(
            f"  已嘗試: {tried_count}/{len(self.all_combinations)} ({tried_count / len(self.all_combinations) * 100:.1f}%)"
        )
        ...
        for pattern, data in self.experience.get("successful_patterns", {}).items():
            ...
            logger.info(
                f"  ✓ {pattern}: {data['success_count']}/{data['total_count']} ({success_rate:.0f}%)"
            )
...
def main():
    ...
    parser.add_argument("--tries", type=int, default=10, help="嘗試次數")
    parser.add_argument(
        "--specific-mode", choices=["ui", "ai", "hybrid"], help="指定模式測試"
    )
        ...
        while True:
            try:
                ...
                if cmd in ("quit", "exit", "q"):
                    ...
                    logger.info(
                        "未知命令。可用命令: random [n], curious [n], smart [n], stats, quit"
                    )
