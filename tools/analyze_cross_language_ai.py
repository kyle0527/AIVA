...
def analyze_cross_language_and_ai():
    ...
    if ai_file.exists():
        ...
        ai_classes = re.findall(r'^class (\w+)\(', content, re.MULTILINE)
    ...
    cross_lang_requirements = {
        "Programming Languages": [
            "Python", "JavaScript", "Go", "Java", "C#", "Ruby", "PHP", "Rust", "C++", "TypeScript"
        ],
        "Web Technologies": [
            "HTML", "CSS", "SASS", "LESS", "React", "Vue", "Angular", "Node.js"
        ],
        "Database Languages": [
            "SQL", "NoSQL", "GraphQL", "MongoDB Query", "Redis Commands"
        ],
        "Infrastructure": [
            "Docker", "Kubernetes", "Terraform", "CloudFormation", "Helm"
        ],
        "Natural Languages": [
    ...
    for enum_file in enum_files:
        ...
        try:
            ...
            enums = re.findall(r'^class (\w+)\(', content, re.MULTILINE)
    ...
    # 檢查是否有語言相關的枚舉
    language_related = [enum for enum in all_enums if any(word in enum.lower() for word in ['lang', 'language', 'locale', 'encoding'])]
    ...
    if ai_file.exists():
        ...
        for category, capabilities in ai_capabilities_needed.items():
            for cap in capabilities:
                # 簡單的關鍵字匹配
                if any(keyword in ai_content.lower() for keyword in cap.lower().split()):
    ...
    return {
        ...
        "multilang_support": len(multilang_found) > 0 if 'multilang_found' in locals() else False,
