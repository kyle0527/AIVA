...
param(
    [Parameter(Mandatory = $false, HelpMessage = "RabbitMQ 服務器主機名或IP地址")]
    ...
    [Parameter(Mandatory = $false, HelpMessage = "RabbitMQ 用戶名")]
    [ValidateNotNullOrEmpty()]
    [ValidateLength(1, 255)]
    [string]$RabbitMQUser = $(if ($env:AIVA_RABBITMQ_USER) { $env:AIVA_RABBITMQ_USER } else { "guest" }),
    
    [Parameter(Mandatory = $false, HelpMessage = "RabbitMQ 密碼 (安全字符串)")]
    [SecureString]$RabbitMQPassword,
    
    [Parameter(Mandatory = $false, HelpMessage = "RabbitMQ 虛擬主機")]
    [ValidateNotNullOrEmpty()]
    [ValidateLength(1, 255)]
    [string]$VHost = $(if ($env:AIVA_RABBITMQ_VHOST) { $env:AIVA_RABBITMQ_VHOST } else { "/" }),
    
    [Parameter(Mandatory = $false, HelpMessage = "啟用詳細輸出")]
    [switch]$Verbose
)
