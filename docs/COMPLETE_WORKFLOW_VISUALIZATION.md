...
- [🔄 模組間依賴關係圖](#模組間依賴關係圖)
- [📋 完整數據格式說明](#完整數據格式說明)
  - [aiva_common 定義的標準格式](#aivacommon-定義的標準格式)
    - [1. Phase 0 輸出](#1-phase-0-輸出)
    - [2. Phase 1 輸出](#2-phase-1-輸出)
    - [3. Phase 2 輸入](#3-phase-2-輸入)
    - [4. Phase 2 輸出](#4-phase-2-輸出)
- [🎯 設計理念總結（完全符合您的規劃）](#設計理念總結完全符合您的規劃)
  - [核心流程](#核心流程)
  - [關鍵設計要點](#關鍵設計要點)
    - [1. **Rust 第一次必須執行**](#1-rust-第一次必須執行)
    - [2. **引擎選擇由 AI 決策**](#2-引擎選擇由-ai-決策)
    - [3. **功能模組接收完整資訊**](#3-功能模組接收完整資訊)
    - [4. **AI 全程分析決策**](#4-ai-全程分析決策)
    - [5. **Integration 持續比對歷史**](#5-integration-持續比對歷史)
    - [6. **RAG 處理未知情況**](#6-rag-處理未知情況)
    - [7. **提前終止機制**（重要！）](#7-提前終止機制重要)
    - [8. **深層漏洞繼續測試**](#8-深層漏洞繼續測試)
  - [完整決策樹](#完整決策樹)
  - [數據流向](#數據流向)
  - [關鍵差異修正總結](#關鍵差異修正總結)
