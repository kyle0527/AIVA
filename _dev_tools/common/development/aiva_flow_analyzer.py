    ...
    def _analyze_script_head_tail(self, node: ScriptNode, script_path: str):
        ...
        for node_ast in ast.walk(tree):
            if isinstance(node_ast, (ast.FunctionDef, ast.AsyncFunctionDef)):
                ...
                # 分析這個函數內部調用的函數
                for child in ast.walk(node_ast):
                    if isinstance(child, ast.Call):
                        if isinstance(child.func, ast.Name):
                            func_info.called_functions.append(child.func.id)
                        elif isinstance(child.func, ast.Attribute):
                            if isinstance(child.func.value, ast.Name):
                                module_name = child.func.value.id
                                called_func = child.func.attr
                                func_info.called_functions.append(f"{module_name}.{called_func}")
        ...
        for func_name in defined_functions:
            is_entry = (func_name not in called_functions or 
                       func_name in ['main', 'run', 'execute', '__init__'])
            if is_entry:
...
class AIVAFlowAnalyzer:
    ...
    def analyze_directory(self, target: str = "core", depth: int = 3,
        ...
        # 1. 產圖階段：掃描並分析所有 Python 檔案（排除分析工具本身）
        ignore_patterns = [
            "__pycache__",
            ".git", 
            ".venv",
            ...
            "_out",
            "_archive",
            "aiva_core_analysis",  # 排除分析結果目錄
            "aiva_flow_analyzer.py",  # 排除分析器本身
            "aiva_flow_classifier",   # 排除分類器
            "analyze_module_distribution.py",  # 排除其他分析工具
            ".pytest_cache",
            "target",
            "build", 
            "dist"
    ...
    def save_results(self, output_dir: str = "aiva_flow_analysis") -> None:
        ...
        # 加入函數映射表
        enhanced_results["function_details"] = {
        ...
        for script_path, script_node in self.stitcher.script_nodes.items():
            script_name = Path(script_path).stem
            enhanced_results["function_details"]["script_functions"][script_name] = {
                "file_path": script_path,
                "functions": {},
            ...
            for func_name, func_info in script_node.functions.items():
                # 全局函數映射
                enhanced_results["function_details"]["function_map"][func_name] = {
                ...
                # 按腳本分組的函數信息
                enhanced_results["function_details"]["script_functions"][script_name]["functions"][func_name] = {
                    "line_number": func_info.line_number,
                    "is_async": func_info.is_async,
                    "is_entry_point": func_info.is_entry_point,
                    "called_functions": func_info.called_functions
